...
/**
 * Tags as pills (finding 2026-09-19, plan Rueckmeldungen P7): a tag is drawn
 * the same way where it is written (live preview) and where it is read, it is
 * exactly what the index counts as a tag, a click opens the tag pane on it, and
 * the device switch "Colour tags" - off by default - colours it by its ROOT.
 * The fixture is the links suite's plain vault; the SQL mock answers the two
 * tag queries of the pane.
 */
const TAG_NOTE = [
  '# Tags',
  '',
  'Work on #project/site and #project/print, maybe an #idea. Issue #42 is a number.',
  '',
  'Not tags: `#code`, [[#Heading]], [text](#heading), word#part.',
  '',
  '```',
  '#fenced',
  '```',
...
async function openTags(page: any) {
  await page.goto('/');
  await expect(page.getByText('Tags', { exact: true }).first()).toBeVisible({ timeout: 20000 });
  await page.getByText('Tags', { exact: true }).first().click();
  await expect(page.locator('.cm-content .pv-tag-pill').first()).toBeVisible({ timeout: 20000 });
}

test('live preview: a pill for exactly what the index counts, the path quieter than the leaf', async ({ page }) => {
  await openTags(page);
  const pills = page.locator('.cm-content .pv-tag-pill');
  await expect(pills).toHaveText(['#project/site', '#project/print', '#idea']);
  // The text is untouched - a mark, not a widget - and carries the tag for the click.
  await expect(pills.nth(0)).toHaveAttribute('data-tag', 'project/site');
  await expect(pills.nth(0).locator('.pv-tag-parent')).toHaveText('#project/');
  // One root, one slot; never the grey slot 0.
  const slots = await pills.evaluateAll((els: Element[]) => els.map((el) => el.getAttribute('data-tag-color')));
  expect(slots[0]).toBe(slots[1]);
  for (const slot of slots) expect(Number(slot)).toBeGreaterThan(0);
  // Off by default: the slot paints nothing, all three look alike.
  const grounds = await pills.evaluateAll((els: Element[]) => els.map((el) => getComputedStyle(el).backgroundColor));
  expect(new Set(grounds).size).toBe(1);
});

test('a click opens the tag pane on the tag, with the way to it unfolded', async ({ page }) => {
  await openTags(page);
  await page.locator('.cm-content .pv-tag-pill', { hasText: 'site' }).click();
  const row = page.locator('[data-tag-row="project/site"]');
  await expect(row).toBeVisible({ timeout: 10000 });
  await expect(row).toHaveAttribute('data-selected', 'true');
  // The notes that carry it are listed below the tree.
  await expect(page.getByText('#project/site', { exact: false }).last()).toBeVisible();
});

test('reading view: the same pill, the same click', async ({ page }) => {
  await openTags(page);
  await page.locator('[data-tip="Lesemodus"], [data-tip="Read Mode"]').first().click();
  const reader = page.locator('.markdown-reader').first();
  const pills = reader.locator('.pv-tag-pill');
  await expect(pills).toHaveText(['#project/site', '#project/print', '#idea'], { timeout: 10000 });
  await expect(reader.locator('code', { hasText: '#code' }).locator('.pv-tag-pill')).toHaveCount(0);
  await pills.nth(2).click();
  await expect(page.locator('[data-tag-row="idea"]')).toHaveAttribute('data-selected', 'true', { timeout: 10000 });
});

test('"Colour tags" on: the root decides the colour, in the note and in the pane', async ({ page }) => {
  await page.addInitScript(() => { (window as any).__tagColors = true; });
  await openTags(page);
  await expect(page.locator('html')).toHaveAttribute('data-tag-colors', 'on', { timeout: 10000 });
  const pills = page.locator('.cm-content .pv-tag-pill');
  const grounds = await pills.evaluateAll((els: Element[]) => els.map((el) => getComputedStyle(el).backgroundColor));
  // Both project tags share a ground; the switch changed it from the neutral one.
  expect(grounds[0]).toBe(grounds[1]);
  const neutral = await page.evaluate(() => getComputedStyle(document.documentElement).getPropertyValue('--chip-tag-bg').trim());
  expect(neutral).not.toBe('');
  const probe = await page.evaluate((value: string) => { const el = document.createElement('span'); el.style.background = value; document.body.appendChild(el); const c = getComputedStyle(el).backgroundColor; el.remove(); return c; }, neutral);
  expect(grounds[0]).not.toBe(probe);
});
