...
describe("desktop comment store coordination", () => {
  let root: string;
  ...
  function makeStore(): CommentStore {
    return createLocalCommentStore(root, new BackupVaultAdapter(raw), async () => "Reviewer", () => {});
  }
  it.each([false, true])("records a first rename through the actual shell store and reopens with late history (locked: %s)", async locked => {
    const mode = vi.spyOn(await import("./encryptionSession"), "hasLocalKeyfile").mockResolvedValue(locked);
