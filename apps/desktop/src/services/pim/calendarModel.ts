 ...
 * Pure; the single source of this rule for every surface that offers a calendar
 * picker (calendar tab, task time blocking). */
...
/**
 * Builds the event draft for "block time" on a task (issue #34, wave 3). Tasks
 * are deliberately day-granular (`PimTaskDraft.due` is a plain date) — an event
 * is the object with a time RANGE, including collision rendering in the grid.
 * So instead of duplicating time planning inside the task model, the task gets
 * an ordinary event that carries its title and links back to the note.
 *
 * The link is a wiki link in the description rather than a provider-private
 * property: it stays readable in Google Calendar and Outlook, and Plainva
 * renders event descriptions as Markdown. Pure.
 */
