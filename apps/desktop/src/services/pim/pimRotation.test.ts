...
vi.mock("@tauri-apps/api/core", () => ({ invoke: async (command: string, args: { key: string; expected: string | null; value: string | null }) => {
  if (command === "keychain_get") return state.raw;
  if (command !== "keychain_compare_and_set") throw Error("unexpected native write");
  if (state.fail) throw Error("native storage unavailable");
  if (state.raw !== args.expected) return false;
  state.raw = args.value; return true;
} }));
