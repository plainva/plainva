/**
 * The sealed comment store: signed objects inside an encrypted workspace,
 * behind the same `CommentStore` contract the open bundle store satisfies
 * (Nachschaerfung, N0).
 *
 * The bodies here used to be six branches inside `VaultContext.tsx`, each
 * beginning with `if (!state.workspaceSecurityStatus)`. The runtime, the
 * state store and the worker stay where they are - moving them would be a
 * different plan - so this is a thin adapter around getters the context hands
 * in, and the context chooses ONE store per vault instead of asking the same
 * question at every call.
 */
import {
...
export class WorkspaceCommentStore implements CommentStore {
  ...
  async post(input: CommentPostInput): Promise<void> {
    ...
    const entry: WorkspaceCommentOutboxEntry = {
      outboxId: createWorkspaceObjectId(), commentId: createWorkspaceObjectId(), path: input.path, targetObjectId: object.objectId,
