...
interface VaultContextType extends VaultState {
  ...
  /** Acts on one entry or a whole group; a retry answers with what is still open (finding 2026-09-03). */
  updateWorkspaceQuarantine: (quarantineIds: string | readonly string[], action: "retry" | "ignore" | "repaired") => Promise<QuarantineRetryOutcome | null>;
  /** Forgets a local fork once it was compared and resolved (C36). */
  discardLocalFork: (forkId: string) => Promise<void>;
  exportWorkspaceQuarantine: (quarantineId: string) => Promise<Uint8Array | null>;
  /** The JSON diagnosis of the given entries (all when none are named): causes, details, times, keys - no ciphertext. */
  exportWorkspaceQuarantineDiagnostics: (quarantineIds?: readonly string[]) => Promise<string>;
  ...
  /** Tries a remark that failed to publish again, right away (K6). */
  retryWorkspaceComment: (outboxId: string) => Promise<void>;
  /** Drops a queued remark that never made it - the person's decision, never the worker's (K6). */
  discardWorkspaceComment: (outboxId: string) => Promise<void>;
  /**
   * Which store serves this vault and how it can hold comments right now
   * (N0/N3): `locked` is the one state the surface has to explain instead of
   * showing an empty list; `hasOutbox` decides whether retry/discard exist.
   */
  getCommentStoreState: () => Promise<CommentStoreState | null>;
  /**
...
/** Whether open suggestions are drawn in the text of this vault's notes (K5). */
export const commentInlineKey = (vaultPath: string) => `commentInline_${btoa(unescape(encodeURIComponent(vaultPath)))}`;
...
export const VaultProvider: React.FC<{
          ...
          if (workspaceSecurityStatus && workspaceStateStore) {
            ...
            if (!runtime) {
              ...
              const startEncryptedWorkspace = async (): Promise<EncryptedWorkspaceWorker | null> => {
                ...
                // A queued remark published or failed, or one arrived from another
                // device: the open column and the overview refresh themselves (K6).
                worker.onCommentsChanged = (paths) => {
                  for (const changedPath of paths) {
                    window.dispatchEvent(new CustomEvent("plainva-workspace-comments-changed", { detail: { path: changedPath } }));
                  }
                };
  ...
  // The service owns retry/ignore/repaired (finding 2026-09-03) - this used
  // to reimplement them and fire a trigger nobody waited for. Only the
  // encrypted worker promises a cycle; a plain sync worker has nothing to
  // re-check and gets the bare trigger.
  const quarantineService = () => {
    const { workspaceState } = workspaceControlPlane();
    const worker = state.syncWorker;
    const runNow = worker && "runNow" in worker ? (worker as { runNow?: () => Promise<void> }).runNow : undefined;
    return new WorkspaceQuarantineService(workspaceState, {
      trigger: () => worker?.triggerImmediate(),
      runNow: typeof runNow === "function" ? () => runNow.call(worker) : undefined,
    });
  };

  const updateWorkspaceQuarantine = async (quarantineIds: string | readonly string[], action: "retry" | "ignore" | "repaired"): Promise<QuarantineRetryOutcome | null> => {
    const service = quarantineService();
  ...
  const discardLocalFork = async (forkId: string): Promise<void> => {
    const { workspaceState } = workspaceControlPlane();
    await workspaceState.deleteLocalFork(forkId);
    window.dispatchEvent(new CustomEvent("plainva-workspace-governance-changed"));
  };

  const exportWorkspaceQuarantine = async (quarantineId: string): Promise<Uint8Array | null> => quarantineService().exportCiphertext(quarantineId);

  const exportWorkspaceQuarantineDiagnostics = async (quarantineIds?: readonly string[]): Promise<string> => {
    const { runtime } = workspaceControlPlane();
    return quarantineService().exportDiagnostics(quarantineIds, { workspaceId: runtime.workspaceId, deviceId: runtime.device.publicIdentity.deviceId });
  };

  /**
   * ONE store per vault (Nachschaerfung, N0): the sealed path inside an
   * encrypted workspace, the sideband bundle everywhere else. This is the only
   * place `workspaceSecurityStatus` decides anything about comments - and
   * there is no fall-back from the sealed store to the open one: a locked
   * workspace throws, it does not quietly write a plaintext bundle beside the
   * signed objects.
   *
   * The open store reads through the BACKUP adapter, exactly what the sync
   * worker hands the sideband step: the conflict-aware app adapter would mint
  ...
  // vault that is not open yet answers that.
  const getWorkspaceCapabilities = async (path: string): Promise<WorkspaceCapability[] | null> => commentStore()?.capabilities(path) ?? null;

  const listWorkspaceComments = async (path: string): Promise<WorkspaceCommentRecord[]> => (await commentStore()?.list(path)) ?? [];
  ...
  const listAllWorkspaceComments = async (): Promise<Map<string, WorkspaceCommentRecord[]>> => (await commentStore()?.listAll()) ?? new Map();
  ...
  /**
   * Who can be named: the policy's members in a workspace, and elsewhere what
   * each device (or named author) calls itself - never a claim about who
   * somebody is. Every consumer reads names only, so one shape serves both.
   */
  const listWorkspaceMembers = async (): Promise<WorkspacePolicyMember[]> => {
    const names = (await commentStore()?.authors()) ?? new Map<string, string>();
    return [...names].map(([memberId, displayName]) => ({ memberId, displayName, state: "active" as const }));
  };

  // Two honest answers to one question, and each store owns its own: the
  ...
  const discardWorkspaceComment = async (outboxId: string): Promise<void> => commentStore()?.discard(outboxId);
  ...
  // Deleting is an appended fact too (K7): the remark stays in the sealed
  // ledger, every device stops showing it. Who may say it is decided by the
  // worker on the receiving side and by the column on this one.
  const retractWorkspaceComment = (path: string, commentId: string) => postWorkspaceCommentRecord(path, "", null, null, null, null, null, commentId);
