      ...
      {!dateProp ? (
        ...
        <>
          <div className="base-period-toolbar">
            <button onClick={prevMonth} className="base-nav-btn" aria-label={t("database.prevPeriod", "Zurück")} data-tip={t("database.prevPeriod", "Zurück")}><ChevronLeft size={ICON.ui} /></button>
          ...
          <div className="custom-scrollbar" style={{ overflowY: "auto", flex: 1 }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7, minmax(0, 1fr))", background: "var(--border-color)", gap: "1px" }}>
              {weekdays.map((d, wi) => (
                <div key={wi} style={{ background: "var(--bg-secondary)", padding: "0.5rem", textAlign: "center", fontWeight: 600, fontSize: "var(--text-sm)", color: "var(--text-muted)" }}>{d}</div>
              ))}
              {days.map((day, i) => {
