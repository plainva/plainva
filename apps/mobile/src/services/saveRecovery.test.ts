...
import { BackupVaultAdapter, ConflictAwareVaultAdapter, ConflictError, QueueingVaultAdapter, SyncQueue, SyncStateRepository, type IDatabaseAdapter } from "@plainva/core";
...
const compiled = ts.transpileModule(
  "let bootPromise = Promise.resolve(vault); const lastPersistedText = new Map();\n" +
...
function harness(vault: MobileVault) {
  ...
  const deps = {
    vault, createSaveCoordinator, ConflictError,
  ...
  const result = new Function(...Object.keys(deps), compiled +
    "\nreturn {noteSaver,switchVault,reloadActiveMobileVault,deleteVault,getLastPersistedText};")(...Object.values(deps)) as {
...
describe("mobile saves through the actual adapter and lifecycle chain", () => {
  ...
  it.each(["switch", "reload", "delete"] as const)("%s retains the current vault after a failed actual write, then allows retry", async (action) => {
    ...
    vi.spyOn(a.raw, "writeTextFile").mockRejectedValueOnce(new Error("disk full"));
