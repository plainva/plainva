...
let cachedDefaults: VaultScopedSettings | null = null;

/**
 * Single source of the per-vault defaults.
 *
 * A FUNCTION rather than a constant on purpose (C20). Every line below reads
 * the shared defaults table across a package boundary, and a table this module
 * builds while it LOADS is exactly the shape that shipped a white window twice:
 * the bundler is free to evaluate this chunk before the one holding the table,
 * and then the whole app dies before it mounts. Inside a function the same
 * reads are safe — by the time anything calls it, every chunk is loaded.
 *
 * Memoised because the answer cannot change at runtime, so callers may treat it
 * like the constant it used to be.
 */
export function vaultDefaults(): VaultScopedSettings {
...
export function pickVault(src: Partial<VaultScopedSettings>): VaultScopedSettings {
  const d = vaultDefaults();
  return {
    dailyFolder: src.dailyFolder ?? d.dailyFolder,
    inboxFolder: src.inboxFolder ?? d.inboxFolder,
    attachmentFolder: src.attachmentFolder ?? d.attachmentFolder,
    templateFolder: src.templateFolder ?? d.templateFolder,
    dailyTemplate: src.dailyTemplate ?? d.dailyTemplate,
    folderTemplates: src.folderTemplates ?? d.folderTemplates,
    typeTemplates: src.typeTemplates ?? d.typeTemplates,
    ...
    backupMaxAgeDays: src.backupMaxAgeDays ?? d.backupMaxAgeDays,
    backupZipEnabled: src.backupZipEnabled ?? d.backupZipEnabled,
    backupZipKeep: src.backupZipKeep ?? d.backupZipKeep,
    syncIntervalSeconds: src.syncIntervalSeconds ?? d.syncIntervalSeconds,
    remindEvents: src.remindEvents ?? d.remindEvents,
    reminderLeadMinutes: src.reminderLeadMinutes ?? d.reminderLeadMinutes,
    reminderAllDayLeadDays: src.reminderAllDayLeadDays ?? d.reminderAllDayLeadDays,
    ...
    mailMailbox: src.mailMailbox ?? d.mailMailbox,
    navigatorTab: src.navigatorTab ?? d.navigatorTab,
    swipeHintSeen: src.swipeHintSeen ?? d.swipeHintSeen,
    verifierName: src.verifierName ?? d.verifierName,
  };
}
