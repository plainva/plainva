...
export function SecurityAreaScreen({ vault, onBack }: { vault: MobileVault; onBack: () => void }) {
  ...
  return <div className="m-page">
    ...
    {runtime ? <>
      ...
      {area === "team" && <>{runtime.policy.payload.members.map((member) => <div className="m-row m-row--static" key={member.memberId}><span>{member.displayName}<small>{member.state} · {member.memberId.slice(0, 12)}</small></span></div>)}{runtime.policy.payload.groups.length > 0 && <p className="m-sectionlabel">{t("workspaceSecurity.groups")}</p>}{runtime.policy.payload.groups.map((group) => <div className="m-row m-row--static" key={group.groupId}><span>{group.name}<small>{(group.memberIds?.length ?? 0)} {t("workspaceSecurity.members")}</small></span></div>)}</>}
      {area === "slices" && <>{runtime.policy.payload.slices.map((slice) => <div className="m-row m-row--static" key={slice.sliceId}><span>{slice.name}<small>{slice.kind} · {slice.materializedObjectIds.length}</small></span></div>)}<p className="m-sectionlabel">{t("workspaceSecurity.publications")}</p><div className="m-row m-row--static"><span><small>{t("workspaceSecurity.mobileManageOnDesktop", { defaultValue: "Manage on the desktop app." })}</small></span></div>{runtime.policy.payload.slices.filter((slice) => slice.publication).map((slice) => <div className="m-row m-row--static" key={`pub-${slice.sliceId}`}><span>{slice.name}<small>{slice.publication?.mode} · {slice.publication?.access}</small></span></div>)}</>}
      </>}
