...
export type EventVisualState = "confirmed" | "tentative" | "cancelled" | "unanswered" | "declined";
 ...
 *
 * 1. `cancelled` — the appointment is off. Nothing else about it matters, so a
 *    cancelled event you had also declined still reads as cancelled.
 * 2. `declined` — you said no. It goes ahead for everyone else, but it is not
 *    part of your day, and that is the thing a calendar has to show at a glance
 *    (maintainer finding 2026-07-30: an accepted and a declined invitation
 *    looked identical). The first cut of this file kept a decline at
 *    `confirmed`, reasoning that the event still exists and the answer belongs
 *    in the dialog. That is true and beside the point: whoever looks at the week
 *    is asking what THEY are doing.
 * 3. `unanswered` — you are invited and have not replied (`needsAction`). It is
 *    not yet an appointment of yours, which is why it gets an outline (E8).
 * 4. `tentative` — either the organiser marked it so, or you replied "maybe".
 *    A "maybe" from you is the same uncertainty as a "maybe" about the event.
 * 5. `confirmed` — the ordinary case, unchanged.
 */
