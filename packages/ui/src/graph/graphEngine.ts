...
export function createGraphScene(
  ...
  function rebuildIndex(): void {
    ...
    // NUL separator (paths never contain it), built at runtime so no raw control
    // byte lands in the source file.
    ...
    for (const e of edges) {
      ...
      // Lane index within an unordered node pair — fans parallel edges apart.
      const pk = e.source < e.target ? e.source + sep + e.target : e.target + sep + e.source;
      const lane = pairSeen.get(pk) ?? 0;
      edgeLane.set(e.id, lane);
      pairSeen.set(pk, lane + 1);
    }
  }
  ...
  /** Quadratic control point for a curved edge — bends perpendicular to the line
   *  by the theme's edgeCurvature, with parallel edges fanning to either side so
   *  a link + a relation between the same two notes no longer coincide. */
  function edgeControl(a: RenderNode, b: RenderNode, id: string): { mx: number; my: number } {
    const dx = b.cx - a.cx;
    const dy = b.cy - a.cy;
    const dist = Math.hypot(dx, dy) || 1;
    const nx = -dy / dist;
    const ny = dx / dist;
    const lane = edgeLane.get(id) ?? 0;
    const laneOffset = lane === 0 ? 0.6 : (lane % 2 === 1 ? -1 : 1) * (0.6 + Math.ceil(lane / 2) * 0.9);
    const bend = tokens.edgeCurvature * dist * laneOffset;
    return { mx: (a.cx + b.cx) / 2 + nx * bend, my: (a.cy + b.cy) / 2 + ny * bend };
  }

  /** Subtle dot grid gives the canvas depth and inherits the theme. */
  function drawDotGrid(w: number, h: number): void {
    if (!ctx || transform.k < 0.2) return;
    // Fade the grid in/out across the zoom threshold instead of a hard cutoff.
    const gridFade = Math.min(1, (transform.k - 0.2) / 0.2);
    const spacing = 28;
  ...
  /** Soft accent halo behind a node — full for interaction states, subtle for
   *  well-connected hubs. Gated on the theme's glowIntensity (0 = flat themes). */
  function drawNodeAura(n: RenderNode, strength: number): void {
    if (!ctx || tokens.glowIntensity <= 0 || strength <= 0.01) return;
    const r = n.size * 2.1;
    const g = ctx.createRadialGradient(n.cx, n.cy, n.size * 0.5, n.cx, n.cy, r);
    g.addColorStop(0, tokens.accentGlowStrong);
    g.addColorStop(1, tokens.accentGlowSoft);
    ctx.globalAlpha = Math.min(1, strength * tokens.glowIntensity);
    ctx.fillStyle = g;
    ctx.beginPath();
  ...
  function draw(): void {
    ...
    // Screen-space vignette — very subtle depth toward the edges (flat themes off).
    if (tokens.glowIntensity > 0) {
      const vg = ctx.createRadialGradient(w / 2, h / 2, Math.min(w, h) * 0.35, w / 2, h / 2, Math.max(w, h) * 0.75);
      vg.addColorStop(0, tokens.accentGlowSoft);
      vg.addColorStop(1, tokens.vignetteTint);
      ctx.fillStyle = vg;
    ...
    // fully hit-testable.
    // Bloom anchor: the hovered node, else (so the highlight PERSISTS after the
    // mouse leaves) the keyboard-focused or single-selected node — click a node
    ...
    for (const e of edges) {
      ...
      // on the rim with no line poking past it (maintainer report 2026-07-14).
      const sdx = ctrl.mx - a.cx;
      const sdy = ctrl.my - a.cy;
      const sl = Math.hypot(sdx, sdy) || 1;
      const startX = a.cx + (sdx / sl) * a.size;
      const startY = a.cy + (sdy / sl) * a.size;
      const edx = b.cx - ctrl.mx;
      const edy = b.cy - ctrl.my;
      const el = Math.hypot(edx, edy) || 1;
      const eux = edx / el;
      const euy = edy / el;
      const arrowed = e.style === "property" && transform.k > 0.35;
      const ah = 8 / transform.k;
      // Leave room for the arrowhead so the line stops before it, not under it.
      const endGap = b.size + (arrowed ? ah * 0.85 : 0);
      const endX = b.cx - eux * endGap;
      const endY = b.cy - euy * endGap;

      // Relation edges get a source->target color gradient; others a single tone.
      if (e.style === "property" && !emphasized && !e.dimmed) {
        const grad = ctx.createLinearGradient(startX, startY, endX, endY);
        grad.addColorStop(0, nodeColor(a));
        grad.addColorStop(1, nodeColor(b));
        ctx.strokeStyle = grad;
      } else {
        ctx.strokeStyle = emphasized ? tokens.accent : edgeStroke(e);
      }
      const base = e.style === "structure" ? 0.7 : e.style === "property" ? 1.4 : 1.1;
      const weightBonus = Math.min(3.5, 0.9 * Math.log2(Math.max(1, e.width)));
      ctx.lineWidth = (base + weightBonus + (emphasized ? 0.7 : 0)) / transform.k;
      ctx.setLineDash(edgeDash(e));
      ctx.beginPath();
      ctx.moveTo(startX, startY);
      ctx.quadraticCurveTo(ctrl.mx, ctrl.my, endX, endY);
      ctx.stroke();
      ctx.setLineDash([]);
      // Directional arrowhead for relations, sitting on the target rim — drawn
      // larger, near-opaque and in the target node's color so it reads clearly.
      if (arrowed) {
        const tipX = b.cx - eux * (b.size + 0.5 / transform.k);
        const tipY = b.cy - euy * (b.size + 0.5 / transform.k);
        ctx.globalAlpha = Math.min(1, ctx.globalAlpha * 1.7 + 0.3);
        ctx.fillStyle = emphasized ? tokens.accent : nodeColor(b);
        ctx.beginPath();
        ctx.moveTo(tipX, tipY);
        ctx.lineTo(tipX - eux * ah - euy * ah * 0.55, tipY - euy * ah + eux * ah * 0.55);
        ctx.lineTo(tipX - eux * ah + euy * ah * 0.55, tipY - euy * ah - eux * ah * 0.55);
        ctx.closePath();
        ctx.fill();
      }
      ...
      if (e.label && (emphasized || transform.k > 0.9)) {
        const mx = 0.25 * a.cx + 0.5 * ctrl.mx + 0.25 * b.cx;
        const my = 0.25 * a.cy + 0.5 * ctrl.my + 0.25 * b.cy;
    ...
    for (const n of visibleSorted) {
      ...
      const interactive = selected || focused || hovered;
      const hubStrength = n.shape !== "folder" && n.size > 11 && !n.dimmed ? Math.min(1, (n.size - 11) / 4) * 0.4 : 0;
      drawNodeAura(n, interactive ? 0.9 : hubStrength * bloomMul);
      ctx.globalAlpha = a0;
      ...
      if (hasIcon) {
        ...
        ctx.arc(n.cx, n.cy, n.size, 0, Math.PI * 2);
        ctx.fill();
        // Specular highlight (top-left) for a touch of depth.
        if (tokens.glowIntensity > 0 && n.size * transform.k > 5) {
          ctx.globalAlpha = a0 * 0.16;
          ctx.fillStyle = tokens.accentOn;
          ctx.beginPath();
          ctx.arc(n.cx - n.size * 0.3, n.cy - n.size * 0.32, n.size * 0.4, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.globalAlpha = a0;
    ...
    for (const n of labelOrder) {
      ...
      if (isFolder) {
        ...
        if (stateShown) {
          ctx.globalAlpha = (n.dimmed ? 0.25 : 1) * 0.9;
          ctx.fillStyle = tokens.bgPrimary;
          ctx.beginPath();
          ctx.roundRect(lx - 4 / transform.k, ly - 2 / transform.k, tw + 8 / transform.k, fontPx * 1.3 + 3 / transform.k, 5 / transform.k);
          ctx.fill();
          ctx.globalAlpha = n.dimmed ? 0.25 : 1;
        }
