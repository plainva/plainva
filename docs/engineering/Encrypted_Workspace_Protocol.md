...
PVE1 is not a compatibility format for encrypted workspace content. Its
content activation path was removed in P3; only settings and account-secret
sideband data continue to use separately keyed PVE1 blobs. They remain a
...
- `apps/desktop/src/services/workspaceSecurity/` and the Security & Sharing
  settings page: OS-keychain/passphrase storage, recovery-save confirmation,
  activation, lock/unlock, diagnostics and explicit old-plaintext cleanup;
...
Activation is deliberately one-way at the protocol level. The Desktop wizard
creates a one-member owner policy, stores the device bundle in the OS keychain
(or a scrypt/XChaCha passphrase envelope), requires an external `.pvrecovery`
save plus random recovery-code group confirmation, then publishes immutable
bootstrap documents. Only after that succeeds does it inventory local files and
enqueue content operations.

The local filesystem remains the plaintext materialisation. Migration records
its complete inventory and counters in SQLite and never deletes or overwrites
the existing provider-side plaintext tree. Once all queued items and the pending
checkpoint/head publication are committed, phase becomes `active`; the Security
Center may then remove the legacy provider objects only after a separate danger
confirmation. Local files are outside that cleanup.

Queue items reserve a signed operation, sequence number, object frame and chunk
set before upload. Staged bytes live below `.plainva/workspace/staging`. A retry
reuses the exact reserved hashes and sequence. Immutable puts make payload and
operation retries idempotent; a head already written immediately before a crash
is adopted only when its bytes match the reserved signed pointer. Incoming
