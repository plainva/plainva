...
- [x] The **installed** binary is signed as well — measured on a real 0.6.8 installation on 2026-08-24: `%LOCALAPPDATA%\Plainva\plainva-desktop.exe` and `uninstall.exe`, both `Valid`, `CN=Marco Kammradt`, both timestamped. This closes the last part that an archive extraction could not answer.
      Two names this list got wrong before, so nobody loses an hour to them again: the binary is **`plainva-desktop.exe`** (the Cargo bin name — `mainBinaryName` is not set, `productName` only affects the display name), and the install directory is **`$env:LOCALAPPDATA\Plainva`**, *not* `…\Programs\Plainva` as guessed here until 2026-08-24. Both errors produce "file not found", which reads exactly like "not signed". Take the path from the registry rather than assuming it:
      `powershell -c "(Get-ItemProperty HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\* | ? DisplayName -like '*lainva*').InstallLocation"`, then `Get-ChildItem <dir> -Filter *.exe | % { Get-AuthenticodeSignature $_.FullName }`. Renaming never invalidates an Authenticode signature — this is about finding the file, not about validity.
      The same run showed the certificate expiring **that very day** (`NotAfter 2026-08-24`), with the signature still `Valid`: the timestamp is what carries it. That is the rule above, observed on a shipped install rather than argued.
