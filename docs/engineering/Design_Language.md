...
Security & Sharing (P3 personal workspace): the page stays inside the quiet-card settings grammar. Setup uses the Modal primitive with `.pv-security-wizard`; its three textual step markers use the accent-container pair only for the active step. The recovery code is a selectable, wrapping `.pv-security-code` well; it is never used as a decorative badge. Identity/status facts use the two-column `.pv-security-details` definition list. Recovery confirmation requires both an OS file save and a separate checkbox before activation. Destructive removal of legacy remote plaintext is a danger action behind the app confirmation service and is unavailable until migration is active.
