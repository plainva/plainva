...
- **Listas de tareas**: si la base de datos es una base de datos de tareas y has conectado una cuenta de calendario/tareas, **Configurar → Fuente de datos** muestra la fila **Crear también nuevas tareas en**. Elige allí una lista y una tarea creada en Plainva también se crea en esa lista en el proveedor; sin elegir nada, sigue siendo solo una nota, como antes. La elección pertenece a la base de datos (guardada como `plainva.taskList`, ver la [Referencia del formato de archivo](File_Format_Reference.md)), no a la tarea individual, y la fila solo aparece cuando una cuenta realmente ofrece una lista de tareas. Si la lista elegida desaparece más tarde (cuenta eliminada, lista eliminada), Plainva no crea la tarea en otro sitio — trata la base de datos como si no se hubiera elegido nada.
