...
- Las entradas del llavero tienen **nombres legibles** — `plainva · <vault> · <servicio> · <id de cuenta> · #<huella>` en lugar de una cadena base64. Plainva renombra las entradas existentes una sola vez, al abrir el vault por primera vez; si un cambio de nombre no puede completarse con seguridad, la entrada antigua se queda donde está y Plainva lo intenta de nuevo en la siguiente apertura.
- **Desconectar** detiene la sincronización del vault; no se elimina ningún archivo en ningún sitio al hacerlo.
- **`http://` está permitido, `https://` es la recomendación.** Un servidor que gestionas tú mismo en tu propia red suele usar `http` sin cifrar — eso funciona, también en el móvil. A través de internet no deberías: WebDAV envía tu contraseña en **cada** solicitud, en texto plano por `http`. Si introduces una dirección sin cifrar fuera de tu propia red, Plainva te lo indica en el formulario — pero no te lo impide.
...
3. **Conectar** valida las credenciales; después elige la **Carpeta en la nube** mediante **Elegir carpeta…**. También aquí se puede crear una **carpeta nueva**, como en todos los sitios donde Plainva elige una carpeta en la nube, también en el móvil bajo **Cambiar carpeta en la nube**.
...
Una nueva conexión de calendario permanece vinculada al vault donde la iniciaste. Si lo cierras o cambias de vault durante la verificación, inicia allí la conexión de nuevo. Si no se pueden leer las credenciales existentes, la combinación se detiene y conserva las cuentas anteriores.
...
Si **Calendario** muestra dos filas para el mismo calendario, Plainva lo señala y **no** las fusiona por su cuenta: fusionarlas costaría la selección del calendario y el enlace a las tareas reflejadas. Comprueba qué fila lleva tu selección y elimina la otra.

Una cuenta que eliminas queda eliminada: la eliminación viaja mediante la sincronización de ajustes a tus otros dispositivos, en vez de volver desde allí en la siguiente ronda.
...
**Las eliminaciones viajan.** Una eliminación que confirmaste queda 90 días en un diario (`.plainva/sync/deletions.json`) y se replica en cualquier otro dispositivo sin preguntar, también tras un reinicio y aunque allí falten muchos archivos a la vez. Si faltan muchos archivos en la nube sin que el diario los explique, Plainva detiene la réplica y pregunta: **aplicar las eliminaciones** o **conservarlas localmente** (entonces vuelve a subir los archivos como nuevos). Lo mismo vale para las listas de tareas: una eliminación hecha en el proveedor también retira la nota de tarea en el otro dispositivo, siempre que allí no haya cambiado. Antes de preguntar —y antes de quitar aquí un solo archivo—, Plainva lo comprueba directamente con la nube: pregunta uno por uno por una muestra de los archivos que faltan. Si tan solo uno de ellos está allí, la lista de la nube estaba incompleta: no se elimina nada, no se pregunta a nadie y el estado de sincronización muestra **La nube respondió de forma incompleta: no se eliminó nada** con **Comprobar de nuevo**. Una entrada del registro solo se escribe después de comprobar que el archivo ya no existe y de quitarlo aquí, y una entrada cuyo archivo vuelve a aparecer en la nube se retira; **Comprobar el registro de eliminaciones**, en **Acerca de y diagnóstico**, lo hace cuando se lo pidas. Si no se encuentra la carpeta en la nube de un vault que ya se ha sincronizado, Plainva ya no crea un sustituto vacío, sino que lo indica: entonces eliges la carpeta existente.
