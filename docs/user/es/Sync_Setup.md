...
**Sincronizar ajustes** transfiere los ajustes compartidos del vault y los metadatos de las cuentas; las rutas locales, el diseño y los datos de ejecución siguen siendo propios de cada dispositivo. **Sincronizar secretos de cuentas** es una opción independiente para contraseñas de aplicación y credenciales BYO permitidas; los tokens OAuth nunca se comparten. El estado de cifrado guía por **Preparación**, **Migración**, **Estricto**, **Descifrado** y **Rotación de clave**; una rotación solo termina después de verificar todos los archivos remotos.

En el **teléfono**, la página de la bóveda muestra lo mismo como una tarjeta de estado: **no se está sincronizando**, **aún no desbloqueado en este dispositivo** (introduce la frase de contraseña — la misma que elegiste en el escritorio) o **se está sincronizando**. Así que la app móvil **desbloquea** un vault cifrado, pero no configura ella misma la sincronización de ajustes. Y como los inicios de sesión nunca viajan, una cuenta sincronizada puede aparecer en un dispositivo que aún no ha iniciado sesión — la app lo indica ahí explícitamente y ofrece iniciar sesión en lugar de quedarse vacía.
