...
La aplicación móvil está en **prueba abierta** en Google Play. En **Android** entras directamente: abre el enlace de prueba desde [plainva.com/android-beta](https://plainva.com/android-beta), toca **Convertirte en tester** e instala la aplicación desde Google Play — sin invitación y sin unirte a ningún grupo. Plainva también está publicada en la Play Store. En **iPhone** y **iPad**, TestFlight es público: abre [https://testflight.apple.com/join/ZRSEfZBn](https://testflight.apple.com/join/ZRSEfZBn) en el dispositivo, instala TestFlight si te lo pide y Plainva se instala desde ahí.
...
- **Todas las áreas** están disponibles en cualquier momento en la **hoja de áreas**: con **Áreas** en la barra o **manteniendo pulsada la barra**. Muestra TU orden —primero las áreas de la barra, luego las demás, tal como las colocaste—, marca la actual y abajo lleva directamente a **Organizar barra de navegación…**. Etiquetas, marcadores y recientes ya no son áreas propias: están bajo **Notas**.
- **Ajustar la barra de navegación:** **Ajustes** → **Barras y áreas**. Con **−**/**+** decides cuántas superficies muestra la barra (2–4, con vista previa en vivo). Cada fila responde: tocarla mete el área en la barra o la saca, el **ojo** al lado indica cuál es el caso y el **asa** reordena arrastrando — en el borde superior e inferior la lista acompaña, así un solo gesto recorre todo. Ocultar no pierde nada: lo que no está en la barra sigue disponible en **Áreas**. Si el área abierta sale de la barra, la app salta a la primera visible. La misma barra se puede organizar **en el escritorio** (Ajustes → Vault → Barras y áreas); con la sincronización de ajustes activada, la disposición viaja entre tus dispositivos.
...
- **＋** flota como un botón redondo sobre la barra y abre la creación rápida en dos grupos: nota, «Desde plantilla…», nota diaria, entrada de diario, carpeta, base de datos — y debajo evento y tarea, que se crean en el calendario y en la lista de tareas. El menú **Nuevo** del escritorio ofrece las mismas entradas en el mismo orden.
...
Justo debajo de la nota diaria está **Diario**: las entradas del día seleccionado, un campo que escribe exactamente en ese día, y **Todos los días**, que abre la pantalla del diario con el flujo de todos los días. **＋** ofrece **Entrada de diario** desde cualquier lugar — ver [Diario](Journal.md).
...
**Lo que ajustas junto a ello.** La **Antelación** se aplica a las citas sin recordatorio propio; **Citas de todo el día** decide en qué tarde o mañana avisan. **Tareas vencidas** incorpora además las tareas de tu base de datos de tareas: con hora, como una cita; sin hora, según la fila **Tareas sin hora** justo debajo, que por defecto recuerda **el día de vencimiento a las 09:00**. **Solo estos calendarios** limita de dónde llegan los recordatorios; si no seleccionas nada, pone **Todos**, y un calendario añadido más tarde entra por sí solo; la hoja permanece abierta hasta que termines, así que puedes marcar varios calendarios de una sentada. La notificación lleva dos acciones: en una cita **Nota de reunión** (la crea o abre la existente), en una tarea **Marcar** — que la completa allí mismo y, en una tarea periódica, crea la siguiente sin que abras la aplicación. Bajo los ajustes, una línea dice además **qué se planificó realmente** — por ejemplo «Programado: 12 citas · 3 tareas» — o por qué no se planificó nada, por ejemplo porque en este dispositivo no hay ninguna base de datos de tareas configurada. Cuando todavía no hay nada que elegir, la fila lo dice en lugar de afirmar **Todos**: **Aún no hay calendarios** cuando hay una cuenta conectada pero aún no ha llegado ningún calendario — al tocarla se ofrece **Actualizar ahora** — y **Ninguna cuenta conectada** mientras no haya ninguna cuenta de calendario configurada.

Los calendarios que tu teléfono ya conoce se conectan sin inicio de sesión: **Ajustes → Cuentas en la nube → Conectar cuenta…** ofrece **En este dispositivo** como primera tarjeta mientras no exista una cuenta del dispositivo. Un toque pide el permiso del sistema; después todos los calendarios del dispositivo están en la lista y, en iOS, las listas de recordatorios como listas de tareas. La tarjeta de la cuenta muestra el estado del permiso y lleva a los ajustes del sistema cuando falta; **Quitar cuenta** solo elimina la conexión, los calendarios del dispositivo se conservan.
...
**Una carpeta existente como bóveda (Android e iOS).** En **Bóvedas → Crear una bóveda** hay una tercera vía, **Una carpeta en este dispositivo**: eliges una carpeta que mantiene otro programa —Syncthing, la app Archivos, un segundo cliente de sincronización— y Plainva lee y escribe ahí sin copiar nada. La carpeta sigue siendo la carpeta: si quitas la bóveda, solo desaparece la conexión, los archivos se quedan. Los cambios que otro programa hace ahí se ven al volver a la app y al abrir una nota. La sincronización en la nube está apagada para estas bóvedas —una segunda sincronización sobre el mismo almacén sobrescribiría la primera— y la tarjeta del detalle lo dice. Si el acceso caduca (carpeta movida, permiso retirado), la tarjeta lo indica y **Volver a conectar la carpeta** lo restaura.
...
Comparte texto, enlaces, imágenes o archivos con Plainva desde otra app. Plainva guarda primero una copia local privada. Abre Plainva, revisa el contenido compartido, elige un vault y una carpeta y confirma la importación. Importar más tarde, cerrar la hoja, bloquear el vault o reiniciar la app conserva el contenido pendiente. Al reintentar una importación iniciada se mantiene su destino original. Si la transferencia de origen está incompleta, vuelve a compartirla; descarta su entrada pendiente cuando ya no la necesites.

Límites por transferencia: 10 archivos, 25 MiB por archivo, 50 MiB en total y 512 KiB de texto incluido el asunto. La cola admite hasta 20 transferencias pendientes y reserva espacio dentro de un presupuesto de adjuntos de 200 MiB. La copia temporal solo se elimina tras escribir y comprobar la nota y los adjuntos, o al descartar expresamente la entrada. Descartar no elimina archivos ya importados al vault. En Android, mantén pulsado el icono de la app para **Nueva nota** y **Hoy**. En escritorio, usa las acciones existentes de pegar e importar archivos.
...
**Desliza una fila hacia la izquierda** para revelar sus acciones: **Marcador** y **Eliminar** en una nota, **Cambiar nombre** y **Eliminar** en una carpeta, **Eliminar** en una base de datos y en el buzón. Son las mismas acciones que la fila ofrece en su menú (mantener pulsado) — el deslizamiento es solo el camino más corto hasta ahí, nunca el único. La primera vez, una franja encima de la lista te lo indica; la descartas con un toque, y aparece exactamente una vez por vault. **En el calendario**, deslizar a la izquierda o a la derecha pasa al período siguiente o anterior (día, tres días, semana, mes); un arrastre que empieza en vertical se desplaza como siempre, y la fila de chips de arriba se desplaza por su cuenta.
...
Android ofrece dos widgets, **Hoy** y **Captura rápida**; en el iPhone hay dos más para la pantalla de bloqueo, un contador y la próxima tarea. Mantén pulsado un hueco libre de la pantalla de inicio y elige Plainva.

**Hoy** muestra lo atrasado como un número y debajo las citas y tareas del día en el mismo orden que la app. Un toque en una fila abre la nota, uno en la cabecera abre la pantalla Hoy y el **+** abre la hoja de captura. **Captura rápida** no muestra contenido alguno: un botón para **Tarea** y otro para **Diario**.

Un widget no calcula nada por sí mismo. Con la app cerrada no se ejecuta nada de Plainva, así que muestra lo que la app dejó escrito la última vez — por eso conoce una semana entera por adelantado y pasa de día a medianoche sin que abras nada. Cuando lo que muestra es de un día anterior, la cabecera dice cuándo se escribió.
