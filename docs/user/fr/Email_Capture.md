...
**Comptes e-mail sans mot de passe.** Une configuration qui échouait à l’enregistrement pouvait laisser une entrée sans mot de passe (Plainva 0.8.3 sur ordinateur). La zone **E-mail** signale une fois les comptes qui n’ont pas de mot de passe sur cet appareil et n’y ont jamais récupéré d’e-mails. **Voir** affiche la liste. Si vous utilisez un tel compte sur un autre appareil, choisissez **Se connecter sur cet appareil** — c’est alors la bonne voie. **Retirer** supprime l’entrée après confirmation, sur tous les appareils qui synchronisent les réglages de ce vault, jamais d’e-mails ni de fichiers. **Plus tard** masque l’avis. Plainva ne retire jamais rien de lui-même.
...
- **Au clavier dans la liste** — Flèche haut/bas sélectionne le message précédent/suivant et l'affiche dans le lecteur — si la flèche arrive sur une conversation repliée, elle la déplie et sélectionne son premier message (vers le bas) ou son dernier (vers le haut) ; dans une conversation dépliée, les flèches parcourent message par message —, Maj+flèche étend la sélection, Début/Fin sautent au premier/dernier message, flèche gauche/droite replie/déplie une conversation, Entrée ouvre le message dans le lecteur, Suppr le met à la corbeille (avec la même confirmation que le bouton). Tous figurent dans la fenêtre des raccourcis (F1) sous **E-mail**.
...
## Se connecter à Gmail avec Google
...
Sélectionnez plusieurs messages pour les marquer comme lus ou non lus, les déplacer ou les supprimer ensemble. Les modifications confirmées actualisent la liste. Les messages non confirmés restent sélectionnés, avec un rapport individuel. L’annulation prend effet après le lot actuel. Rechargez le dossier avant de réessayer une action au résultat incertain. Si le serveur ne permet pas une action ciblée, Plainva l’explique et préserve les autres messages.

**Avec pièces jointes** filtre les messages chargés. L’avis indique combien disposent de métadonnées connues. Des métadonnées absentes ne signifient pas l’absence de pièces jointes ; les pages suivantes peuvent contenir d’autres résultats.

Sur un clavier matériel mobile, Haut/Bas et Début/Fin déplacent le focus dans la liste. Entrée ouvre ou sélectionne le message. Gauche/Droite replient ou déplient les conversations ; Échap quitte la sélection ou annule les lots suivants. Les champs de recherche conservent leurs touches habituelles.
