...
- **Arborescence de fichiers** : clic droit → **Nouvelle base de données (.base)** — ou via le bouton **Nouveau** de la barre latérale (**Nouvelle base de données (.base)**).
...
- **Jour d'après le nom du fichier** (`file.day`) — les calendriers et les frises peuvent placer les notes quotidiennes d'après leur **nom de fichier**, sans que la date figure aussi dans le frontmatter. Le champ apparaît dans le sélecteur de champ de date dès que le coffre nomme des notes quotidiennes (dossier et format sous **Réglages → Votre coffre → Contenu et structure**) ; il est calculé à partir du nom, ne figure dans aucune note et ne peut pas être modifié — une carte placée ainsi ne peut donc pas être glissée vers un autre jour (cela renommerait le fichier). Obsidian ne connaît pas ce champ : une vue qui l'utilise y reste vide.
...
- **Couleur de carte** : une carte du Kanban est teintée avec la couleur propre de la note — **Couleur** dans le menu de la carte la définit ; c'est la même couleur que montrent l'en-tête de la note et sa carte du tableau d'affichage. **Couleur de carte selon** dans les paramètres **Vue** teinte les autres cartes avec la couleur d'option d'une propriété Sélection/Statut/Sélection multiple ; la couleur propre d'une note est prioritaire.
- **Liste de tâches sur la carte** : une carte dont la note contient des lignes `- [ ]` affiche une barre de progression avec le décompte ; cliquer dessus déplie les lignes, vous les cochez directement là ou ajoutez une sous-tâche dans le champ en dessous (elle se place après la dernière case de la note). Le décompte est aussi la colonne **Liste de tâches**, disponible dans chaque vue sous **Propriétés**.
- **Couloirs** : sous **Vue** (sur le téléphone : **Configurer**), **Couloirs par** ajoute un second axe — une ligne par valeur d'une autre propriété de sélection, de statut ou de relation (priorité, responsable…), avec les mêmes colonnes dans chaque ligne. Déposer une carte sur une cellule écrit à la fois la propriété de la colonne et celle du couloir ; la flèche replie un couloir, et Plainva s'en souvient par fichier. Sur le téléphone, les couloirs s'empilent, et la puce de couloir sur une carte la déplace vers un autre couloir.
- **Limite de WIP** : cliquez sur le décompte dans l'en-tête de colonne et saisissez le nombre maximal de cartes que la colonne doit contenir — l'en-tête affiche alors `6/5` dans la couleur d'avertissement dès que la colonne le dépasse. Les paramètres **Vue** (sur le téléphone : **Configurer**) listent les limites de toutes les colonnes ; vide signifie aucune limite.
