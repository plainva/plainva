...
- **Listes de tâches** : si la base de données est une base de tâches et que vous avez connecté un compte calendrier/tâches, **Configurer → Source de données** affiche la ligne **Créer aussi les nouvelles tâches dans**. Choisissez-y une liste, et une tâche créée dans Plainva est aussi créée dans cette liste chez le fournisseur ; sans ce choix, elle reste une simple note, comme avant. Le choix appartient à la base de données (stocké sous `plainva.taskList`, voir la [Référence du format de fichier](File_Format_Reference.md)), pas à la tâche individuelle, et la ligne n'apparaît que lorsqu'un compte propose effectivement une liste de tâches. Si la liste choisie disparaît ensuite (compte supprimé, liste supprimée), Plainva ne crée pas la tâche ailleurs — elle traite la base de données comme si rien n'avait été choisi.
