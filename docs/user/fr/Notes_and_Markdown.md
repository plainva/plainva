...
- **Backlinks** : la section **Backlinks** dans la barre latérale droite montre quelles notes renvoient à la note active — regroupées par fichier source, avec un compteur pour les occurrences multiples. Sous chaque fichier, chaque occurrence est listée avec les titres et éléments de liste qui la précèdent et la ligne elle-même ; un clic y mène directement. Sur le téléphone, la feuille de contexte de la note montre les mêmes emplacements. Les lignes sont classées par **Titre** ; le bouton de tri placé au-dessus passe à **Dernière modification** ou **Nombre de liens**, choisir à nouveau la même clé inverse le sens, et le choix est mémorisé sur cet appareil.
...
- **Tags** : `#projet` — ou imbriqué, `#projet/site` — n'importe où dans le texte est un tag. Il s'affiche comme une petite pastille à l'écriture comme à la lecture, et un clic dessus ouvre les notes qui le portent (sur le téléphone : un appui en mode lecture). Ce qui compte comme tag et comment les tags prennent une couleur : [Recherche](Search.md).
...
Sont protégés les **champs système OKF** gérés par Plainva : `type` (la valeur se choisit dans une liste déroulante de types connus, le nom/type de champ/la suppression sont verrouillés) et — là où d'anciennes notes le portent encore — `okf_version` (affichage seul ; depuis l'OKF 0.2, le champ n'a sa place que dans l'`index.md` racine). Contexte : [OKF](OKF.md).

### Statut, avis de péremption et marque de relecture (OKF 0.2)

Cinq propriétés facultatives de l'OKF 0.2 permettent à une note de dire d'où elle vient et si elle tient toujours — Plainva les affiche sans aucun réglage :

- **Badge de statut :** si une note porte `status: draft` ou `status: deprecated`, l'en-tête du document affiche le badge **Brouillon** ou **Obsolète**. `stable` reste silencieux. Une colonne `status` à vous avec d'autres valeurs (disons `Open`) n'est pas concernée — c'est une propriété ordinaire et elle n'obtient aucun badge.
- **Avis de péremption :** une fois `stale_after` passé, **Marquée comme périmée (depuis le …)** apparaît au-dessus de la note avec **Ouvrir les propriétés**. L'avis ne change rien dans la note ; il ne fait que rappeler.
- **Confiance et provenance :** une section à part dans le panneau des propriétés résume `generated` (Générée), `verified` (Vérifiée), `sources` (Sources, cliquables), `status` (Statut) et `stale_after` (Périmée après) et en déduit un niveau — **Non vérifiée**, **Confirmée par la machine** ou **Relue par une personne**.
- **Marquer comme relue :** le bouton de cette section ajoute votre nom avec l'instant présent à la liste verified (`human:<nom>`) ; Plainva demande le nom une fois par vault et le garde uniquement sur cet appareil (modifiable sous **Paramètres → Vault → Contenu et structure → Nom du relecteur**). Sur le téléphone, la même action se trouve dans la fiche contextuelle de la note.

Plainva ne définit `generated` et `sources` que là où il crée les notes lui-même — dans l'importeur, dans la capture d'e-mails et dans la synchronisation des tâches ; l'éditeur ne tamponne jamais, et les notes existantes ne sont pas tamponnées après coup. Le contrat de champ se trouve dans la [Référence du format de fichier](File_Format_Reference.md), le contexte sous [OKF](OKF.md).
...
Une note quotidienne peut aussi contenir un **journal** : de courtes entrées avec une heure (`- 14:05 Texte`) sous le titre **Journal**, écrites avec `Ctrl+Shift+J` ou **Entrée de journal**, et lues sur tous les jours dans la vue du journal. Ce sont de simples lignes de liste — voir [Journal](Journal.md).
...
- **Exporter en Markdown…** (menu **⋮** de l'éditeur ou palette de commandes) : enregistre une copie de la note n'importe où via la boîte de dialogue système — par exemple pour la transmettre à un autre programme. Les pièces jointes liées (images) ne sont pas copiées ; si la note en référence, Plainva affiche un bref avis. Si la note porte des annotations ouvertes, Plainva demande d'abord **Inclure les annotations ?** — **En liste à la fin (lisible partout)** ou **Marquées dans le texte (CriticMarkup)** ; les marqueurs d'ancrage invisibles disparaissent dans tous les cas.
...
- **Le son se lit** : un fichier audio intégré (`.m4a`, `.mp3`, `.wav`, `.ogg`, `.opus`, `.webm`, `.flac`, `.aac`) affiche un lecteur au lieu de son nom — dans l'éditeur, en mode lecture, sur les cartes du tableau et dans les lignes du journal. Un son joint est donc intégré comme une image (`![[…]]`) et non simplement lié. Plainva ne lit pas de vidéo ; un `.mp4` reste un lien.
...
- [OKF](OKF.md) — ce que signifient `type`, la version du bundle et les champs d'OKF 0.2
...
## Inclure des sections et des blocs
