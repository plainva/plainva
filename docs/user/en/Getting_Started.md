...
- **Vault** — the selected vault sits as a small card in the rail (the active vault carries a dot); with several vaults, **Switch** below it opens a picker list. Below that, five areas per vault: **Sync** (see [Sync Setup](Sync_Setup.md)), **Calendar & accounts** (calendar and email accounts, see [Calendar & Tasks](Calendar_and_Tasks.md) and [Email capture](Email_Capture.md)), **Content & structure** (**Daily notes**, **Templates & tasks** including the **Template Folder**, **OKF (Open Knowledge Format)** — see [OKF](OKF.md) — and **Extended databases**), **Backup & version history** and **Maintenance** (**Rebuild index**, restore deleted files, vault statistics).
