...
**Linux: which file to pick.** On Fedora, openSUSE and other RPM-based distributions install the `.rpm`, on Debian and Ubuntu the `.deb` — both use the WebKitGTK, Mesa and Wayland libraries of your system. The `.AppImage` is the fallback for distributions without a package: it carries its own copies of some libraries, and on a very new system (Mesa 25 or newer) those can clash with the host's graphics stack. AppImages built after 0.8.0 work around that at start-up; if a window still stays blank, start it from a terminal and read the message — `EGL_BAD_PARAMETER` points to exactly this, and the `.rpm` or `.deb` is the way past it. A blank window on an NVIDIA card is a different matter: try `WEBKIT_DISABLE_DMABUF_RENDERER=1 ./Plainva*.AppImage` (a WebKitGTK setting, see [Tauri's notes on Linux graphics](https://v2.tauri.app/develop/debug/linux-graphics/)).
...
- **App** — everything that applies app-wide, in five areas. **Appearance**: the **Theme** picker as preview cards — besides **Petrol** (the default) you get **Nord**, **Solarized**, **Gruvbox**, **Catppuccin**, **Paper** (E-Ink-like, maximally calm), **Sepia** (warm paper), **Forest**, **Midnight** (OLED black), **High Contrast** and **Phosphor Green**/**Phosphor Amber** (retro terminal with subtle scanlines); plus the **Mode** (**Light**/**Dark**/**System Default**; single-mode themes such as **Midnight** pin the mode, and the light/dark switch in the title bar pauses while they are active), **Language**, **Week starts on**, **Density** and **Interface zoom**, and the **Fonts** card with three slots — **Interface**, **Content** and **Code** — plus the **Content font size**; each slot offers **Theme default**, serif, sans-serif, monospace or **Custom…** (the **Code** slot monospace only), and behind the custom choice sits a list in which every face previews itself and says whether this device actually has it; a font that is not installed cannot be picked, and a free-text field stays below it for everything else. Fonts are kept per device. **Editor & notes**: **Default view**. **Startup & behavior**: open the last vault automatically, compatibility warnings. **Updates**: Plainva quietly checks for new versions on startup and shows a notice when one is found — click it to download and install the update straight away (it stays up until Plainva restarts). Opt out via **Check for updates on startup**. **About & diagnostics**: version details, the **OS keychain** status, **Performance metrics**, **Export diagnostics…** (no note content) and **Report a problem**. Keyboard shortcuts stay reachable any time via `F1` or **Show keyboard shortcuts** in the bottom left.
...
- **Colour tags**: under **Settings → App → Appearance**, switch on **Colour tags** and every tag gets a colour that follows from its name — nested tags share the colour of their top-level tag — in notes, properties, databases and the tag list. Off by default; it applies to this device and stores nothing.
