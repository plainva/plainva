...
- **On this device** (phone only): the calendars your device already knows — iCloud, Exchange, Google through the system accounts, local calendars — as **one** account with no login. A tap asks for the system permission (full access, because Plainva creates events too); on iOS the **reminder lists** join as task lists, Android has no system task store. There is at most one device account per phone, it does not travel through the settings sync, and the same calendar may also be connected through a cloud account — Plainva does not merge the two.
...
**Connecting an account twice does not add a second one.** When the provider confirms it is the same account — Plainva goes by the provider's **verified identity**, never by the displayed name; two people at one company easily share one — the new sign-in is adopted into the account you already have instead of appearing beside it. The calendar selection, the sync cursors and above all the links between your task notes and the tasks at the provider survive that; without them the next sync would create every mirrored task a second time. With CalDAV, where there is no verified identity, a separate entry appears as before.
...
Open it from the left action rail (calendar icon) or the command palette (**Open calendar**). Five views are available via the switch in the header: **Day**, **3 days** and **Week** show a **time grid** with an hour gutter on the left; events sit as blocks at their start time, their height is the duration, overlapping events sit side by side, and a red line marks "now". All-day events and (with the task overlay on) due tasks sit in the strip above the grid. **Month** shows the month grid (one colored dot per calendar) plus a single-day time grid for the selected day on the right. **Agenda** lists the upcoming weeks grouped by day. **Today** jumps back; the arrows page by the current period (a day, three days, a week or a month). The first day of the week follows the **Week starts on** setting (Settings → App → Appearance: Monday, Saturday or Sunday) — it also applies to the sidebar calendar. The view refreshes automatically every few minutes; the refresh button forces it. For Microsoft calendars and most CalDAV servers Plainva now fetches only the **changes** instead of the whole range every time — noticeably less data, especially on the phone. A full refresh still runs at least once an hour so nothing lingers that a change feed missed; and nothing is ever deleted except what the provider explicitly reports as deleted. With Google every refresh is a full one: its API does not allow change lists together with a date range. Events that have already ended read **dimmer** (like Google Calendar), so today's remaining agenda stands out. A **multi-day event** is one continuous **bar** across the days it covers — one label, one click target, instead of an entry per day. Where it runs past the end of the week it is cut straight at the edge and continues in the next row without repeating the title. The all-day strip of the day, three-day and week views works the same way. **Clicking the title** (or `Tab` to it and `Enter`) opens the date picker: year, month, day, **Today** — a day jumps there; the arrow keys move inside the grid, `Esc` closes. On a trackpad a horizontal two-finger swipe (or `Shift` + mouse wheel) over the calendar pages the same way — one period per swipe.

The keyboard hints below the date picker wrap when space is limited; year navigation, all seven weekdays and **Today** stay fully inside the picker.

- **Create an event**: **clicking an empty slot in the time grid** opens a small quick-create popover (title, time, calendar, location) — **Save** creates it right away, **More options** opens the full event dialog. **Dragging** across the grid sets the duration. The **+** in the header opens the full dialog: title, calendar, date/time or an all-day range, location, a **description** (a formatting editor — Markdown, "/" for commands; formatted descriptions from Google/Outlook read as text rather than raw HTML code, and a formatted description is sent formatted too), a **color**, **attendees**, and an optional Outlook-style **repeat**. The color overrides the calendar's color for that single event (no effect on Microsoft accounts — Outlook has no per-event colors). On the phone the event colour is picked from the same grid in the event sheet.
...
**When a task's state does not arrive.** If Plainva does not show what you changed at the provider — a task you ticked off, a date you moved — the provider's own data settles the question. Switch on **Record raw task data** under **Settings → App → About & diagnostics** (on the phone: **About & diagnostics**), sync, make the change at the provider, sync again, and then use **Export diagnostics…**: the export lists both pulls, each task with its id, status, dates and revision — titles cut to 16 characters, no notes, no credentials. Switch it off afterwards; that also deletes the recording.

## Reminders on the computer

Under **Settings → Calendar → Reminders** you switch on **Remind me of appointments**; the first time, the system asks once for permission. Whatever reminder the appointment itself carries wins — only when it says nothing does the **Lead time** apply, and all-day appointments speak up at the time chosen under **All-day appointments**. **Due tasks** additionally takes in the tasks of your task database, and **Only these calendars** narrows down where reminders come from (nothing ticked means all of them, and a calendar connected later is included by itself). Right below it sits **Tasks without a time**: a task is due ON its day, not the evening before — so it has a rule of its own (day and time freely chosen, by default **on the due day at 09:00**). It used to borrow the all-day appointment rule, which meant it spoke up at 19:00 the night before and never again, with nothing anywhere saying why. A task whose due value carries a **time** reminds at that time, with a lead of its own under **Tasks with a time** (by default **At the time**) — it used to borrow the appointment's lead time. In the note, `remind: off` or `remind: 30` makes an exception for one task. On the phone the reminder offers **Tick off**, **In 1 hour** and **Tomorrow morning**; the two "later" buttons move this one reminder on this device and leave the task untouched. The **All-day appointments** row now takes any hour too, instead of two fixed combinations; both rows write the resulting sentence out underneath.
...
On the phone this includes the device's **reminder lists** (iOS) once the device account **On this device** is connected: each list is a task list like a Nextcloud list, with the same rules — import into the task database, checking off in both directions, deletion through the deletion journal.
