...
- **Task lists**: If the database is a task database and you have connected a calendar/tasks account, **Configure → Data source** shows the row **Also create new tasks in**. Pick a list there and a task created in Plainva is also created in that list at the provider; without a choice it stays a note, exactly as before. The choice belongs to the database (stored as `plainva.taskList`, see the [file format reference](File_Format_Reference.md)), not to the individual task, and the row only appears when an account actually offers a task list. If the chosen list later disappears (account removed, list deleted), Plainva does not create the task somewhere else — it treats the database as if nothing had been chosen.
