...
Right under the daily note sits **Journal**: the entries of the selected day, a field that writes into exactly that day, and **All days**, which opens the journal screen with the stream of all days. **＋** offers **Journal entry** from anywhere — see [Journal](Journal.md).
...
**Swipe a row left** to reveal its actions: **Bookmark** and **Delete** on a note, **Rename** and **Delete** on a folder, **Delete** on a database and in the mailbox. They are the same actions the row offers in its menu (long press) — the swipe is the shorter way there, never the only one. The first time, a line above the list says so; you tap it away, and it appears exactly once per vault. **In the calendar**, a swipe left or right pages to the next or previous period (day, three days, week, month); a drag that starts vertically scrolls as usual, and the chip row above scrolls on its own.
