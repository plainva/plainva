...
- **Place:** off by default. Switch on **Allow adding a place to journal entries** (Settings → Appearance) and the capture field gains a second button; one press adds your coordinates to the entry as its own line: `📍 52.5200, 13.4050`. Four decimals is about eleven metres — enough for the building, not for the room. You can write the name of the place yourself or delete the line; Plainva asks no online service for it. **Nothing is recorded silently**: no history, no map, no background tracking — only that one position at the moment you press the button. The system asks for permission on the first press. On the phone the button is always available; on a computer only where the operating system gives the app a position — otherwise it is absent rather than always failing.
