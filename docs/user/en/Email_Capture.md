...
Plainva can read your mailbox — and only read it — to get knowledge out of email and into your vault. It is deliberately **not** a mail client: it connects via IMAP in read-only mode, never changes anything in the mailbox (not even the unread markers), and never sends mail itself.

## Connecting a mailbox

**Settings → your vault → Calendar & accounts → Email (IMAP, read-only) → Add account…**: host, port and an **app password**. For Gmail that is `imap.gmail.com`, port `993`, with an app password from [myaccount.google.com/apppasswords](https://myaccount.google.com/apppasswords) (requires 2-factor authentication) — no OAuth, no verification. Connecting validates the login before anything is saved; the password goes into your operating system's keychain. The **Mail folder** setting chooses where captured emails are stored (default `Mail`).
