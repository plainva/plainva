...
Cada ocorrência mostra duas linhas: **antes** com o trecho encontrado e **depois** com o resultado — com expressão regular, as referências `$1` são resolvidas, para conferir a mudança antes de gravar qualquer coisa. Uma expressão inválida é apontada no próprio campo em vez de uma lista vazia; se nada for encontrado, o estado vazio diz o que conferir. Durante a substituição você vê o progresso e pode **Cancelar** — notas já gravadas continuam gravadas e são nomeadas. No celular cada ocorrência mostra as mesmas duas linhas.

**No telefone** o mesmo está em a lupa no cabeçalho e então `>` e **Localizar e substituir no vault**: as ocorrências ficam agrupadas por nota e recolhidas, para que um termo com quarenta ocorrências não soterre a ação; toque numa nota para ver por dentro, desmarque as que quiser deixar de fora, e o botão anuncia o próprio alcance (**Substituir em 2 notas**). Ao sair do app, uma substituição em andamento para na próxima nota — as notas já gravadas permanecem gravadas e são nomeadas.
...
Na nota, uma tag é desenhada como uma pequena pílula — enquanto você escreve e enquanto lê; o texto em si continua sendo `#projeto/site`. Um clique na pílula (no telefone: um toque no modo de leitura) abre as notas que carregam a tag. O que conta como tag é igual em todo lugar — na lista de tags, em uma tarefa e ao renomear: um `#` no início da linha ou depois de um espaço, seguido de letras, dígitos, `_`, `-` ou `/`. Apenas dígitos não são uma tag (`#42` continua sendo um número), nem o que está dentro de código ou de um link. **Colorir tags** em **Configurações → App → Aparência** dá a cada tag uma cor que decorre do nome; tags aninhadas compartilham a cor da tag de nível superior. A configuração pertence a este dispositivo e não salva nada nas suas notas.
