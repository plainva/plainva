...
- **Listas de tarefas**: se o banco de dados for um banco de tarefas e você tiver conectado uma conta de calendário/tarefas, **Configurar → Fonte de dados** mostra a linha **Criar também novas tarefas em**. Escolha uma lista ali e uma tarefa criada no Plainva também é criada nessa lista no provedor; sem essa escolha, ela continua sendo apenas uma nota, exatamente como antes. A escolha pertence ao banco de dados (armazenada como `plainva.taskList`, veja a [Referência do Formato de Arquivo](File_Format_Reference.md)), não à tarefa individual, e a linha só aparece quando uma conta realmente oferece uma lista de tarefas. Se a lista escolhida desaparecer depois (conta removida, lista excluída), o Plainva não cria a tarefa em outro lugar — ele trata o banco de dados como se nada tivesse sido escolhido.
