...
- **O dia termina às** (mesmo lugar nas configurações) empurra o limite do dia para mais tarde: com **04:00**, tudo o que você escrever entre meia-noite e as quatro ainda pertence ao dia anterior — a entrada vai para a nota diária de ontem e mantém o seu horário real (`- 01:30 …`). O cabeçalho do dia no diário diz então **até 04:00**. O limite vale para a nota diária e para o diário, **não** para o calendário nem para o vencimento das tarefas: um compromisso à 01:30 de quarta continua na quarta. O padrão é **Meia-noite**; a configuração pertence ao cofre e vale em todos os dispositivos.
- **Humor:** se a configuração **Humor: propriedade da nota diária** (mesmo lugar) tiver um nome — `humor`, por exemplo —, o cabeçalho do dia no diário mostra cinco pontos e você avalia o dia com um toque. A nota diária passa a guardar `humor: 4`; pressionar o valor atual o apaga. A mesma propriedade pode ser uma coluna do tipo **Avaliação** num banco e ser ordenada ali. Deixe vazio e este cofre não avalia dias.
- **Nota de voz**: o ícone do microfone no campo de captura grava. Enquanto corre você vê o tempo decorrido e tem duas saídas: **Descartar** joga a gravação fora, **Anexar** a escreve na pasta de anexos e a adiciona à entrada. O nome do arquivo leva a data e a hora (`Nota de voz 2026-09-22 1430.m4a`). O Plainva pede acesso ao microfone no **primeiro** toque, nunca ao iniciar, e não grava nada enquanto você não começar; a gravação fica no seu cofre e não vai a lugar nenhum.
- **Lugar:** desativado por padrão. Ative **Permitir adicionar um lugar às entradas do diário** (Configurações → Aparência) e o campo de captura ganha um segundo botão; um toque adiciona suas coordenadas à entrada em uma linha própria: `📍 52.5200, 13.4050`. Quatro casas decimais são cerca de onze metros — bastam para o prédio, não para a sala. O nome do lugar você mesmo pode escrever, ou apagar a linha; o Plainva não pergunta a nenhum serviço on-line. **Nada é registrado em silêncio**: sem histórico, sem mapa, sem rastreamento em segundo plano — apenas aquela posição no momento em que você toca. O sistema pede permissão no primeiro toque. No telefone o botão está sempre lá; num computador, só onde o sistema operacional dá uma posição ao aplicativo.
