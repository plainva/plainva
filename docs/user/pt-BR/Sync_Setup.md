...
**Sincronizar configurações** transfere as configurações compartilhadas do vault e os metadados das contas; caminhos locais, layout e dados de execução permanecem específicos do dispositivo. **Sincronizar segredos das contas** é uma opção separada para senhas de app estáticas e credenciais BYO permitidas; tokens OAuth nunca são compartilhados. O estado da criptografia orienta por **Preparação**, **Migração**, **Estrito**, **Descriptografia** e **Rotação de chave**.

No **celular**, a página do vault mostra o mesmo como um cartão de estado: **as configurações não estão sendo sincronizadas**, **Ainda não desbloqueado neste dispositivo** (digite a frase secreta que você escolheu no desktop) ou **as configurações estão sendo sincronizadas**. Assim, o app mobile **desbloqueia** um vault criptografado, mas não configura a sincronização de configurações por conta própria. E como os logins nunca viajam, uma conta sincronizada pode aparecer em um dispositivo que ainda não fez login — o app diz isso explicitamente e oferece o login em vez de ficar vazio.
