...
Uma vez definido, a visualização de Tarefas mostra duas seções: no topo, as entradas do **Banco de tarefas** (com status e data de vencimento; **Abrir como banco de dados** leva à visualização completa do banco de dados, com seu quadro e filtros), e abaixo, **Das notas** — a familiar lista de caixas de seleção.
