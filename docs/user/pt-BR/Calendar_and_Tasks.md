...
**Conectar uma conta duas vezes não adiciona uma segunda.** Quando o provedor confirma que é a mesma conta — o Plainva se baseia na **identidade verificada** do provedor, nunca no nome exibido; duas pessoas na mesma empresa facilmente compartilham um — o novo login é incorporado à conta que você já tem, em vez de aparecer ao lado dela. A seleção de calendários, o estado da sincronização e, acima de tudo, os vínculos entre suas notas de tarefa e as tarefas no provedor sobrevivem a isso; sem eles, a próxima sincronização criaria cada tarefa espelhada uma segunda vez. Com o CalDAV, onde não existe identidade verificada, aparece uma entrada separada como antes.
...
- **Editar / excluir**: **Editar evento** na prévia abre o diálogo pré-preenchido com seus valores e com as ações **Nota de reunião** e **Excluir**. As alterações são gravadas no provedor com uma verificação de segurança: se o evento mudou remotamente nesse meio-tempo, o Plainva atualiza a visualização em vez de sobrescrever. Em um **evento único**, o diálogo oferece ainda um **seletor de calendário** — escolha outro calendário e o evento é **movido** para lá (criado no destino, excluído da origem; ele recebe um novo identificador do provedor).
...
Listas de tarefas novas vêm **marcadas desde o início** — ao contrário dos calendários, que você escolhe primeiro. Desmarcar permanece: após um reinício, após atualizar as listas e através da sincronização de configurações.
...
**Isso vale nos dois aparelhos.** A sincronização também roda no celular — as tarefas são importadas lá, notas existentes são reconhecidas em vez de criadas em duplicidade, e suas edições são enviadas ao provedor. A única diferença está na janela de desfazer: no desktop, fechar o Plainva cancela a exclusão; no celular, é enviar o app para segundo plano que faz isso. De um jeito ou de outro, a tarefa permanece.
...
**Quando o estado de uma tarefa não chega.** Se o Plainva não mostra o que você mudou no provedor — uma tarefa marcada como concluída, uma data movida —, os dados do próprio provedor resolvem a questão. Ative **Registrar dados brutos de tarefas** em **Configurações → App → Sobre e diagnóstico** (no telefone: **Sobre e diagnóstico**), sincronize, faça a mudança no provedor, sincronize de novo e então use **Exportar diagnóstico…**: a exportação lista as duas consultas, cada tarefa com identificador, status, datas e revisão — títulos cortados em 16 caracteres, sem notas, sem credenciais. Depois desative; isso também apaga o registro.
