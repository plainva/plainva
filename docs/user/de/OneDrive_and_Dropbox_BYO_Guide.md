...
**Normalerweise brauchst Du diese Seite nicht:** Für OneDrive **und** Dropbox bringt Plainva eine eigene App-ID mit — Du wählst den Anbieter, klickst **Verbinden** und meldest Dich an. Eine **eigene** (kostenlose) App-Registrierung brauchst Du nur in zwei Fällen: wenn eine Firmen-Sperre den mitgelieferten Zugang blockiert, oder wenn Du die Verbindung bewusst unter Deine eigene Kontrolle stellen willst. Am **Desktop** blendest Du die Felder in den Sync-Einstellungen über **Eigene App-ID verwenden** ein; am **Telefon** steht das Feld direkt im Verbinden-Formular unter dem Anbieter. Trag dort genau einen öffentlichen Wert ein:
...
> Für beide Anbieter ist die mitgelieferte ID bereits hinterlegt — Teil A und Teil B brauchst Du nur für eine **eigene** Registrierung.
