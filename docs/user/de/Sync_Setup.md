...
3. **Verbinden** prüft die Zugangsdaten; danach wählst Du über **Ordner auswählen…** den **Cloud-Ordner**. Auch hier lässt sich ein **neuer Ordner** anlegen — wie überall, wo Plainva einen Cloud-Ordner wählt, auch mobil unter **Cloud-Ordner ändern**.
...
Der Sync-Fehlerdialog bewahrt den genauen fehlgeschlagenen Versuch, auch wenn ein automatischer Retry den Live-Status bereits verändert hat. Er zeigt an, wenn der erneute Versuch läuft oder erfolgreich war. Eine neue Anmeldung wird nur bei einem Authentifizierungsfehler empfohlen; Netzwerk-, Timeout- und Providerfehler behalten ihre konkrete Ursache und werden automatisch erneut versucht. Auch der Abgleich der Einstellungen wartet vorübergehende Fehler ab: Eine Zeitüberschreitung erscheint zunächst als ruhiger Hinweis mit Zähler und wird erst nach dem dritten Fehlschlag in Folge zur roten Meldung — eine abgelaufene Anmeldung dagegen sofort.

## Namen, die sich nur in der Schreibweise unterscheiden

Google Drive unterscheidet beim Suchen keine Groß- und Kleinschreibung, und Windows und macOS legen `Notiz.md` und `notiz.md` in derselben Datei ab. Dasselbe Zeichen kann außerdem auf zwei Arten gespeichert sein: `ü` als ein Zeichen oder als `u` mit angehängtem Trema. Bei dieser **Schreibweise** meint Plainva jetzt dieselbe Datei und synchronisiert normal weiter, solange es nur eine Entsprechung gibt.
