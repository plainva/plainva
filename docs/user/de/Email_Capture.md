...
**E-Mail-Konten ohne Passwort.** Eine Einrichtung, die beim Speichern scheiterte, konnte einen Eintrag ohne Passwort zurücklassen (Plainva 0.8.3 am Desktop). Der Bereich **E-Mail** weist einmal auf Konten hin, die auf diesem Gerät kein Passwort haben und hier noch nie Mails abgerufen haben. **Ansehen** zeigt die Liste. Nutzt Du ein solches Konto auf einem anderen Gerät, wähle **Auf diesem Gerät anmelden** — das ist dann der richtige Weg. **Entfernen** löscht den Eintrag nach einer Rückfrage auf allen Geräten, die die Einstellungen dieses Vaults abgleichen, nie Mails oder Dateien. **Später** blendet den Hinweis aus. Von sich aus entfernt Plainva nichts.
...
- **Breite Nachrichten werden eingepasst** — viele Newsletter sind für eine feste Spaltenbreite gebaut und lassen sich nicht umbrechen. Statt eine solche Nachricht an der linken Kante abzuschneiden, verkleinert Plainva sie auf die Breite des Rahmens; auf dem Telefon wächst der Rahmen dabei mit, sodass Du wie gewohnt die Seite scrollst.
...
- **Mit der Tastatur durch die Liste** — Pfeil hoch/runter wählt die vorherige/nächste Nachricht und zeigt sie im Leser — trifft die Taste auf eine zugeklappte Konversation, klappt sie diese auf und wählt ihre erste (abwärts) bzw. letzte (aufwärts) Nachricht; durch eine offene Konversation laufen die Pfeiltasten von Nachricht zu Nachricht —, Umschalt+Pfeil erweitert die Auswahl, Pos1/Ende springen an den Anfang/das Ende, Pfeil links/rechts klappt eine Konversation zu/auf, Enter öffnet die Nachricht im Leser, Entf verschiebt sie in den Papierkorb (mit derselben Nachfrage wie der Knopf). Alle Kürzel stehen auch im Tastenkürzel-Fenster (F1) unter **E-Mail**.
...
Wähle mehrere Nachrichten aus, um sie gemeinsam als gelesen oder ungelesen zu markieren, zu verschieben oder zu löschen. Bestätigte Änderungen werden sofort übernommen. Nicht bestätigte Nachrichten bleiben ausgewählt; der Bericht nennt jede Nachricht und ihren Zustand. Abbrechen stoppt nach dem laufenden Paket. Bei unklarem Ergebnis lädst Du den Ordner vor einer erneuten Aktion neu. Unterstützt der Server das gezielte Verschieben oder Löschen nicht, erhältst Du eine Erklärung; andere Nachrichten werden nicht mit gelöscht.

**Mit Anhängen** berücksichtigt die bereits geladenen Nachrichten. Der Hinweis zeigt, für wie viele davon Metadaten vorliegen. Eine Nachricht ohne Metadaten gilt nicht als anhanglos; weitere Seiten können zusätzliche Treffer enthalten.

Auf einer mobilen Hardwaretastatur bewegen Pfeil hoch/runter sowie Pos1/Ende den Fokus in der Mailliste. Enter öffnet die Nachricht oder wählt sie im Auswahlmodus aus. Links/rechts klappt Konversationen ein oder aus; Escape beendet die Auswahl oder bricht weitere Pakete ab. Suchfelder behalten ihre normalen Tasten.
