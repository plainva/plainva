...
- **Links auf einen Abschnitt**: `[[Notiz#Überschrift]]` öffnet die Notiz an dieser Überschrift, `[[#Überschrift]]` springt innerhalb der aktuellen Notiz, und `[Text](#überschrift)` im GitHub-Stil tut dasselbe — der wörtliche Überschriftentext und der Slug (`cool-header`) werden beide aufgelöst; `[[Notiz#^id]]` erreicht eine Zeile, die auf `^id` endet. Beim Tippen von `[[Notiz#` werden die Überschriften der Notiz angeboten. Eine Überschrift, die es nicht mehr gibt, meldet sich mit einer Nachricht, statt nichts zu tun.
...
- **Tags**: `#projekt` — oder verschachtelt `#projekt/website` — irgendwo im Text ist ein Tag. Es steht als kleine Pille da, beim Schreiben wie beim Lesen, und ein Klick darauf öffnet die Notizen, die es tragen (am Telefon: ein Tipp im Lesemodus). Was als Tag zählt und wie Tags Farbe bekommen: [Suche](Search.md).
...
- Im Icon-Modus wählst Du oben eine **Farbe** — dieselbe Palette wie beim Farbstreifen, **A** für die Standardfarbe, die Scheibe mit dem Farbring (**Eigene Farbe …**) für einen freien Wert; sie gilt für das Icon, das Du anschließend antippst. Dasselbe Raster findest Du überall, wo Plainva eine Farbe fragt — auch am Telefon.
...
- **Ton wird abgespielt**: Eine eingebettete Tondatei (`.m4a`, `.mp3`, `.wav`, `.ogg`, `.opus`, `.webm`, `.flac`, `.aac`) zeigt einen Abspieler statt ihres Namens — im Editor, im Lesemodus, auf Pinnwand-Karten und in Journal-Zeilen. Angehängter Ton wird deshalb wie ein Bild eingebettet (`![[…]]`) und nicht nur verlinkt. Filme spielt Plainva nicht ab; eine `.mp4` bleibt ein Link.
