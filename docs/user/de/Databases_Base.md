...
- **Tag aus dem Dateinamen** (`file.day`) — Kalender und Zeitleiste können Tagesnotizen nach ihrem **Dateinamen** einsortieren, ohne dass das Datum zusätzlich im Frontmatter steht. Das Feld erscheint im Wähler für das Datumsfeld, sobald der Vault Tagesnotizen benennt (Ordner und Format unter **Einstellungen → Dein Vault → Inhalt & Struktur**); es wird aus dem Namen berechnet, steht in keiner Notiz und lässt sich nicht bearbeiten — eine Karte, die so platziert ist, lässt sich deshalb auch nicht auf einen anderen Tag ziehen (das würde die Datei umbenennen). Obsidian kennt dieses Feld nicht: eine Ansicht, die es benutzt, bleibt dort leer. Der Preis dafür, das Datum nicht zweimal zu führen.
...
- **Kartenfarbe**: Eine Board-Karte wird mit der eigenen Farbe der Notiz getönt — **Farbe** im Menü der Karte setzt sie; es ist dieselbe Farbe, die der Notizkopf und die Pinnwand-Karte zeigen. **Kartenfarbe nach** in den **Ansicht**-Einstellungen tönt die übrigen Karten mit der Optionsfarbe einer Auswahl-/Status-/Mehrfachauswahl-Eigenschaft; die eigene Farbe einer Notiz hat Vorrang.
...
- **WIP-Limit**: Klick auf die Zählung im Spaltenkopf und die Höchstzahl an Karten eintippen — der Kopf zeigt dann `6/5` in der Warnfarbe, sobald die Spalte sie überschreitet. Die **Ansicht**-Einstellungen (am Telefon: **Konfigurieren**) führen die Limits aller Spalten; leer heißt kein Limit.
...
- **Kommentare an einer Zelle**: Trägt eine Eigenschaft Anmerkungen, zeigt ihre Zelle einen kleinen Punkt mit der Zahl der offenen Stränge — in **Tabelle**, **Board** und **Galerie**. Ein Kommentar hängt dabei an der **Notiz und ihrem Eigenschaftsschlüssel**, nicht an der `.base`: dieselbe Anmerkung erscheint deshalb in jeder Datenbank, die die Notiz zeigt, und im **Eigenschaften**-Panel der Notiz selbst. **Eigenschaft kommentieren** im Zellenmenü beginnt einen neuen Strang. Beim Umbenennen einer Spalte ziehen die Kommentare mit; wird die Eigenschaft gelöscht, bleibt die Karte lesbar und nennt den damals notierten Wert. Mehr dazu unter [Sicherheit & Freigaben](Security_and_Sharing.md).
