...
- **Strom oder Karten:** Der Umschalter im Kopf der Ansicht — **Strom** und **Karten** — zeigt dieselben Einträge in zwei Formen. Der Strom liest einen Tag nach unten, die Kartenwand lässt eine Woche überblicken; jeder Tag behält seine eigene Wand mit einer Trennlinie darüber. Karten können dasselbe wie Zeilen: antippen öffnet die Stelle in der Notiz, Rechtsklick beziehungsweise langes Drücken öffnet dieselben Aktionen, Aufgaben-Kästchen haken ab. Die Wahl merkt sich **das Gerät** — am Telefon der Strom, am großen Bildschirm die Wand, wenn Du es so willst.
...
- **Stimmung:** Trägt die Einstellung **Stimmung: Eigenschaft der Tagesnotiz** (gleiche Stelle) einen Namen — etwa `stimmung` —, dann zeigt der Tageskopf im Journal fünf Punkte, und Du bewertest den Tag dort mit einem Klick. In der Tagesnotiz steht danach `stimmung: 4`; ein Klick auf den aktuellen Wert löscht ihn wieder. Dieselbe Eigenschaft lässt sich in einer Datenbank als Spalte vom Typ **Bewertung** führen und dort sortieren. Leer lassen heißt: dieser Vault bewertet keine Tage, und der Tageskopf zeigt nichts.
...
- **Ort:** Standardmäßig aus. Schaltest Du **Ort an Journal-Einträge anhängen können** ein (Einstellungen → Darstellung beziehungsweise Erscheinungsbild), erscheint im Erfassungsfeld ein zweiter Knopf; ein Druck hängt Deine Koordinaten als eigene Zeile an den Eintrag: `📍 52.5200, 13.4050`. Vier Nachkommastellen sind etwa elf Meter — genug für das Gebäude, nicht für den Raum. Den Namen des Orts kannst Du selbst dazuschreiben oder die Zeile löschen; Plainva fragt keinen Online-Dienst danach. Es wird **nichts still mitgeschrieben**: kein Verlauf, keine Karte, keine Hintergrund-Ortung — nur diese eine Position in dem Moment, in dem Du den Knopf drückst. Das System fragt beim ersten Druck nach der Freigabe. Am Telefon ist der Knopf immer verfügbar; am Rechner nur dort, wo das Betriebssystem der App eine Position gibt — sonst fehlt er, statt immer zu scheitern.
