...
- **Taaklijsten**: als de database een takendatabase is en je een agenda-/takenaccount hebt gekoppeld, toont **Configureren → Gegevensbron** de rij **Nieuwe taken ook aanmaken in**. Kies daar een lijst en een taak die in Plainva wordt aangemaakt, wordt ook in die lijst bij de provider aangemaakt; zonder keuze blijft het gewoon een notitie, precies als voorheen. De keuze hoort bij de database (opgeslagen als `plainva.taskList`, zie de [Bestandsformaat-referentie](File_Format_Reference.md)), niet bij de individuele taak, en de rij verschijnt alleen wanneer een account ook echt een taaklijst aanbiedt. Verdwijnt de gekozen lijst later (account verwijderd, lijst verwijderd), dan maakt Plainva de taak niet ergens anders aan — het behandelt de database dan alsof er niets was gekozen.
