...
- Installatie vind je onder **Instellingen → Vault → Cloudaccounts**: **Account verbinden…** opent de assistent — kies eerst de **provider** (**Microsoft**, **Google**, **Nextcloud**, **Dropbox**, **Objectopslag (S3)** of **WebDAV / CalDAV**), vink dan de **diensten** aan (voor bestandssync: **Bestanden**), en meld je vervolgens aan. **Precies één** account per vault draagt de dienst **Bestanden**. Het gebied **Synchronisatie** toont daarna het verbonden account met zijn **Cloudmap** en regelt het gedrag (**Sync-interval**, wachtrij); **Account beheren** leidt terug naar de cloudaccounts.
