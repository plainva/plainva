...
3. **Verbinden** controleert de toegangsgegevens; kies daarna de **Cloudmap** via **Map kiezen…**. Ook hier kun je een **nieuwe map** aanmaken — zoals overal waar Plainva een cloudmap kiest, op de telefoon ook onder **Cloudmap wijzigen**.
...
Kort samengevat: kies in **Cloudaccounts** → **Account verbinden…** de tegel **Google**, vink de dienst **Bestanden** aan, voer de **Client-ID** en **Client secret** uit je Google-project in, en dan **Aanmelden met Google…** — de aanmelding opent in je browser. Eenmaal verbonden kies je de **Cloudmap** via **Map kiezen…** rechtstreeks uit je Drive (submappen inbegrepen, standaard "Plainva"). Let op: zolang je Google-project in de **Testing**-modus staat, verloopt de aanmelding na **7 dagen** — definitief, want Google laat in deze modus ook het vernieuwingstoken verlopen; Plainva kan haar op de achtergrond dus niet verversen. De synchronisatie zegt je dan dat de aanmelding verlopen is, en **Opnieuw aanmelden** in de accountdetails herstelt haar — één doorloop voor **alle** diensten van dit account. **In productie** verwijdert de vaste vervaltijd van de testmodus. Dit garandeert geen blijvende aanmelding: Google kan toegang nog steeds laten verlopen of intrekken. De eisen voor publicatie en verificatie hangen af van het gebruik en de gevraagde rechten. [Google: vervallen van vernieuwingstokens](https://developers.google.com/identity/protocols/oauth2#expiration).
...
**Verwijderingen reizen mee.** Een verwijdering die je hebt bevestigd staat 90 dagen in een logboek (`.plainva/sync/deletions.json`) en wordt op elk ander apparaat zonder vraag overgenomen — ook na een herstart, en ook als daar veel bestanden tegelijk ontbreken. Ontbreken er daarentegen veel bestanden in de cloud zonder dat het logboek ze verklaart, dan pauzeert Plainva het spiegelen en vraagt: **verwijderingen overnemen** of **lokaal behouden** (dan uploadt het de bestanden opnieuw als nieuw). Hetzelfde geldt voor takenlijsten: een bij de aanbieder uitgevoerde verwijdering ruimt de taaknotitie ook op het andere apparaat op, zolang die daar onveranderd is. Voordat Plainva het vraagt — en voordat het hier ook maar één bestand weghaalt — controleert het rechtstreeks bij de cloud: het vraagt één voor één naar een steekproef van de ontbrekende bestanden. Staat er ook maar één, dan was de lijst van de cloud onvolledig: er wordt niets verwijderd, niemand krijgt een vraag en de synchronisatiestatus toont **De cloud antwoordde onvolledig — er is niets verwijderd** met **Opnieuw controleren**. Een journaalvermelding wordt pas geschreven nadat een bestand aantoonbaar weg was en hier is weggehaald, en een vermelding waarvan het bestand weer in de cloud opduikt, wordt ingetrokken; **Verwijderlogboek controleren** onder **Over & diagnose** doet dat op verzoek. Wordt de cloudmap van een vault die al eerder heeft gesynchroniseerd niet gevonden, dan maakt Plainva geen lege vervanger meer aan maar meldt het dat — je kiest dan de bestaande map.
