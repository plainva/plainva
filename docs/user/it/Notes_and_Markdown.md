...
- **Link a una sezione**: `[[Nota#Titolo]]` apre la nota a quel titolo, `[[#Titolo]]` salta dentro la nota corrente, e `[testo](#titolo)` in stile GitHub fa lo stesso — si risolvono sia il testo letterale del titolo sia lo slug (`cool-header`); `[[Nota#^id]]` raggiunge una riga che termina con `^id`. Digitando `[[Nota#` vengono proposti i titoli della nota. Un titolo che non esiste più lo dice con un messaggio invece di non fare nulla.
...
- **Backlink**: la sezione **Backlink** nella barra laterale destra mostra quali note collegano quella attiva — raggruppate per file sorgente, con un contatore per le occorrenze multiple. Sotto ogni file è elencata ogni occorrenza con le intestazioni e le voci di elenco che la precedono e la riga stessa; un clic porta esattamente lì. Sul telefono il foglio di contesto della nota mostra gli stessi punti. Le righe sono ordinate per **Titolo**; il pulsante di ordinamento sopra di esse passa a **Ultima modifica** o **Numero di collegamenti**, scegliere di nuovo la stessa chiave inverte la direzione, e la scelta viene ricordata su questo dispositivo.
...
- **Tag**: `#project` — o annidato, `#project/website` — in qualsiasi punto del testo è un tag. È disegnato come una piccola pillola mentre scrivi e mentre leggi, e un clic su di esso apre le note che lo portano (sul telefono: un tocco in modalità lettura). Che cosa conta come tag e come i tag ricevono un colore: [Ricerca](Search.md).
...
Protetti sono i **campi di sistema OKF** gestiti da Plainva: `type` (il valore è selezionabile da un menu a tendina di tipi noti, nome/tipo di campo/eliminazione sono bloccati) e — dove note più vecchie lo portano ancora — `okf_version` (solo visualizzazione; da OKF 0.2 il campo appartiene solo all'`index.md` radice). Approfondimento: [OKF](OKF.md).

### Stato, avviso di obsolescenza e marchio di revisione (OKF 0.2)

Cinque proprietà opzionali di OKF 0.2 permettono a una nota di dichiarare da dove viene e se è ancora valida — Plainva le mostra senza alcuna configurazione:

- **Badge di stato:** se una nota porta `status: draft` o `status: deprecated`, l'intestazione del documento mostra il badge **Bozza** o **Dismessa**. `stable` resta silenzioso. Una tua colonna `status` con altri valori (ad esempio `Open`) non viene toccata — è una proprietà ordinaria e non riceve alcun badge.
- **Avviso di obsolescenza:** una volta superato `stale_after`, sopra la nota compare **Segnata come obsoleta (dal …)** con **Apri le proprietà**. L'avviso non cambia nulla nella nota; ricorda soltanto.
- **Fiducia e provenienza:** una sezione a sé nel pannello delle proprietà riassume `generated` (Generata), `verified` (Verificata), `sources` (Fonti, cliccabili), `status` e `stale_after` (Obsoleta dopo) e ne ricava un livello: **Non verificata**, **Confermata dalla macchina** o **Rivista da una persona**.
- **Segna come revisionata:** il pulsante in quella sezione aggiunge il tuo nome con l'istante corrente all'elenco `verified` (`human:<nome>`); Plainva chiede il nome una volta per vault e lo tiene solo su questo dispositivo (modificabile in **Impostazioni → Vault → Contenuto e struttura → Nome del revisore**). Sul telefono la stessa azione si trova nel pannello di contesto della nota.

Plainva imposta `generated` e `sources` solo dove crea le note lui stesso — nell'importatore, nella cattura e-mail e nella sincronizzazione delle attività; l'editor non timbra mai da solo, e le note esistenti non vengono timbrate a posteriori. Il contratto dei campi è nella [File Format Reference](File_Format_Reference.md), l'approfondimento sotto [OKF](OKF.md).
...
- In modalità icone scegli un **colore** in alto: la stessa tavolozza della fascia dell'intestazione, **A** per il colore predefinito e il disco con l'anello di tonalità (**Colore personalizzato …**) per un valore libero; vale per l'icona che toccherai subito dopo. La stessa griglia compare ovunque Plainva chieda un colore, anche sul telefono.
...
- **Esporta come Markdown…** (menu **⋮** dell'editor o palette dei comandi): salva una copia della nota in qualsiasi posizione tramite la finestra di dialogo di sistema — ad esempio per consegnarla a un altro programma. Gli allegati collegati (immagini) non vengono copiati insieme; se la nota ne referenzia, Plainva mostra un breve avviso. Se la nota ha annotazioni aperte, Plainva chiede prima **Includere le annotazioni?** — **Come elenco alla fine (leggibile ovunque)** o **Marcate nel testo (CriticMarkup)**; i marcatori di ancoraggio invisibili vengono rimossi in ogni caso.
...
- **Il suono si riproduce**: un file audio incorporato (`.m4a`, `.mp3`, `.wav`, `.ogg`, `.opus`, `.webm`, `.flac`, `.aac`) mostra un lettore invece del suo nome — nell'editor, in modalità lettura, sulle schede della bacheca e nelle righe del diario. L'audio allegato viene quindi incorporato come un'immagine (`![[…]]`) e non solo collegato. Plainva non riproduce video; un `.mp4` resta un collegamento.
...
- [OKF](OKF.md) — cosa significano `type`, la versione del bundle e i campi di OKF 0.2
