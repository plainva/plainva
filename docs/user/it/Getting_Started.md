...
Le **Impostazioni** (icona a forma di ingranaggio in basso nella barra delle azioni all'estrema sinistra, o `Ctrl+,`) si chiudono con la **X** in alto a destra, `Esc` o un clic fuori dalla finestra. Le modifiche vengono salvate subito e automaticamente — solo le credenziali di sincronizzazione vengono applicate deliberatamente tramite **Salva**/**Connetti** (vedi [Configurare la sincronizzazione](Sync_Setup.md)). Le impostazioni si dividono in due parti; ogni area nella barra laterale sinistra apre la propria pagina, dove le impostazioni si trovano in schede di gruppo denominate:
...
- **Vault** — il vault selezionato è mostrato come una piccola scheda nella barra laterale (il vault attivo porta un punto); con più vault, **Cambia** sotto di essa apre un elenco di selezione. Sotto, cinque aree per vault: **Sincronizzazione** (vedi [Configurare la sincronizzazione](Sync_Setup.md)), **Calendario e account** (calendario e account e-mail, vedi [Calendario e attività](Calendar_and_Tasks.md) e [Cattura e-mail](Email_Capture.md)), **Contenuto e struttura** (**Note giornaliere**, **Modelli e attività** inclusa la **Cartella dei modelli**, **OKF (Open Knowledge Format)** — vedi [OKF](OKF.md) — e **Database estesi**), **Backup e cronologia delle versioni** e **Manutenzione** (**Ricostruisci indice**, ripristina i file eliminati, statistiche del vault).
