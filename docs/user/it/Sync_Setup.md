...
## Confronta le versioni conservate

Quando un conflitto di testo non può essere unito automaticamente, rimane aperta una sola copia di lavoro locale per originale. Le modifiche successive vengono salvate nella stessa copia, anche dopo aver riaperto la nota o riavviato l’app. L’avviso spiega: **Stai modificando la tua copia in conflitto. Verrà sincronizzata dopo l’unione.**
...
- La configurazione si trova in **Impostazioni → Vault → Account cloud**: **Collega account…** apre l'assistente — scegli prima il **provider**, poi spunta i **servizi** (per la sincronizzazione dei file: **File**), poi accedi. La panoramica a schede elenca i provider in base alla diffusione reale; con **Cerca provider…** trovi anche i provider di posta disponibili come preimpostazione. **Esattamente un** account per vault porta il servizio **File**. L'area **Sincronizzazione** mostra quindi l'account collegato con la sua **Cartella cloud** e gestisce il comportamento (**Intervallo di sincronizzazione**, coda); **Gestisci account** riporta agli account cloud. Appena il servizio **File** è collegato, l'assistente chiede una volta la **cartella cloud** — per ogni provider, con **Nuova cartella** direttamente nel selettore; **Sfoglia** resta la via per tornarci. Con WebDAV la cartella fa parte dell'indirizzo del server, quindi lì non c'è selettore.
...
3. **Collega** verifica le credenziali; scegli poi la **Cartella cloud** tramite **Scegli cartella…**. Anche qui si può creare una **nuova cartella** — come ovunque Plainva scelga una cartella cloud, anche sul telefono sotto **Cambia cartella cloud**.
