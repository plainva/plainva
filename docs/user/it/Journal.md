...
- **Umore:** se l'impostazione **Umore: proprietà della nota giornaliera** (stesso punto) porta un nome — per esempio `umore` —, l'intestazione del giorno nel diario mostra cinque punti e valuti la giornata con una pressione. La nota giornaliera conserva poi `umore: 4`; premere il valore attuale lo cancella. La stessa proprietà può essere una colonna di tipo **Valutazione** in un database e vi si può ordinare. Lascia vuoto e questo vault non valuta le giornate.
...
- **Luogo:** disattivato per impostazione predefinita. Attiva **Consenti di aggiungere un luogo alle voci del diario** (Impostazioni → Aspetto) e il campo di acquisizione guadagna un secondo pulsante; una pressione aggiunge le tue coordinate alla voce su una riga propria: `📍 52.5200, 13.4050`. Quattro decimali sono circa undici metri: bastano per l'edificio, non per la stanza. Il nome del luogo puoi scriverlo tu o cancellare la riga; Plainva non lo chiede a nessun servizio online. **Nulla viene registrato in silenzio**: nessuna cronologia, nessuna mappa, nessun tracciamento in background, solo quella posizione nel momento in cui premi. Il sistema chiede il permesso alla prima pressione. Sul telefono il pulsante c'è sempre; su un computer solo dove il sistema operativo dà una posizione all'app.
