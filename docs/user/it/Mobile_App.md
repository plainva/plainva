...
L’app mobile è in **test aperto** su Google Play. Su **Android** si entra direttamente: apri il link del test da [plainva.com/android-beta](https://plainva.com/android-beta), tocca **Diventa tester** e installa l’app da Google Play — nessun invito e nessun gruppo a cui iscriversi. Plainva è pubblicato anche sul Play Store. Su **iPhone** e **iPad** TestFlight è pubblico: apri [https://testflight.apple.com/join/ZRSEfZBn](https://testflight.apple.com/join/ZRSEfZBn) sul dispositivo, installa TestFlight se te lo chiede e Plainva si installa da lì.

**Requisiti di sistema:** su iPhone e iPad Plainva richiede **iOS 16.4** o successivo — lì il motore che disegna l’interfaccia fa parte del sistema, e un Safari più recente non lo cambia. Su Android basta Android 7, ma **Android System WebView** deve essere aggiornato; se è troppo vecchio, Plainva lo dice all’avvio e indica la strada dal Play Store.
...
- **Tutte le aree** restano raggiungibili dal **foglio delle aree**: con **Aree** nella barra o **tenendo premuta la barra**. Mostra la TUA disposizione — prima le aree della barra, poi le altre, nell'ordine che hai scelto —, segna quella corrente e in fondo porta direttamente a **Disponi la barra di navigazione…**. Tag, segnalibri e recenti non sono più aree a sé: stanno sotto **Note**.
- **Impostare la barra di navigazione:** **Impostazioni** → **Barre e aree**. Con **−**/**+** decidi quante superfici mostra la barra (2–4, con anteprima dal vivo). Ogni riga risponde: toccarla porta l'area nella barra o la toglie, l'**occhio** accanto dice come sta, e la **maniglia** riordina trascinando — ai bordi superiore e inferiore l'elenco scorre insieme, così basta un movimento. Nascondere non perde nulla: ciò che non è nella barra resta raggiungibile da **Aree**. Se l'area aperta esce dalla barra, l'app salta alla prima visibile. La stessa barra si dispone anche **sul desktop** (Impostazioni → Vault → Barre e aree); con la sincronizzazione delle impostazioni attiva, la disposizione viaggia fra i tuoi dispositivi.
...
- **＋** fluttua come un pulsante rotondo sopra la barra e apre la creazione rapida in due gruppi: nota, "Da modello…", nota giornaliera, voce di diario, cartella, database — e sotto evento e attività, creati nel calendario e nell'elenco attività. Il menu **Nuovo** del desktop offre le stesse voci nello stesso ordine.
...
Proprio sotto la nota giornaliera si trova **Diario**: le voci del giorno selezionato, un campo che scrive esattamente in quel giorno, e **Tutti i giorni**, che apre la schermata del diario con il flusso di tutti i giorni. **＋** offre **Voce di diario** da qualsiasi punto — vedi [Diario](Journal.md).
...
Ogni corrispondenza mostra due righe — **prima** con il punto trovato, **dopo** con il risultato, riferimenti `$1` risolti con un’espressione regolare — così puoi verificare la modifica prima che venga scritto qualcosa.
...
**Che cosa imposti insieme.** L'**Anticipo** vale per gli appuntamenti senza un promemoria proprio; **Appuntamenti di un giorno intero** stabilisce in quale sera o mattina si fanno sentire. **Attività in scadenza** aggiunge anche le attività del tuo database delle attività — con un orario come un appuntamento, senza orario secondo la riga **Attività senza orario** subito sotto, che per impostazione predefinita ricorda **nel giorno di scadenza alle 09:00**. **Solo questi calendari** limita da dove arrivano i promemoria; se non selezioni nulla compare **Tutti**, e un calendario aggiunto in seguito rientra da sé; il foglio resta aperto finché non hai finito, così spunti più calendari in una volta sola. La notifica porta due gesti: su un appuntamento **Nota della riunione** (la crea oppure apre quella esistente), su un'attività **Spunta** — che la completa lì per lì e, per un'attività ricorrente, crea la successiva senza che tu apra l'app. Sotto le impostazioni, una riga dice anche **cosa è stato effettivamente pianificato** — per esempio «Pianificato: 12 appuntamenti · 3 attività» — oppure perché non è stato pianificato nulla, per esempio perché su questo dispositivo non è impostato alcun database attività. Quando non c’è ancora nulla da scegliere, la riga lo dice invece di affermare **Tutti**: **Ancora nessun calendario** quando un account è collegato ma non è ancora arrivato alcun calendario — un tocco lì propone **Aggiorna ora** — e **Nessun account collegato** finché non è configurato alcun account calendario.

I calendari che il tuo telefono conosce già si collegano senza accesso: **Impostazioni → Account cloud → Collega account…** offre **Su questo dispositivo** come prima tessera finché non esiste un account del dispositivo. Un tocco chiede il permesso di sistema; poi tutti i calendari del dispositivo sono nell'elenco e, su iOS, gli elenchi di promemoria come elenchi attività. La scheda dell'account mostra lo stato del permesso e porta alle impostazioni di sistema quando manca; **Rimuovi account** elimina solo il collegamento, i calendari del dispositivo restano.
...
In seguito puoi aprire **E-mail** come area a sé dal **foglio delle aree** e collocarla nella barra di navigazione. La riga sotto il titolo mostra cartella, non letti e account, e apre il selettore delle cartelle. Tocca un messaggio per leggerlo; **Salva come nota** lo archivia nella cartella **Mail** del tuo vault (catturarlo due volte apre la stessa nota). Le immagini remote restano bloccate finché non le consenti per quel messaggio: un’immagine caricata rivela al mittente quando e dove hai letto. Le quattro azioni — **Rispondi**, **Rispondi a tutti**, **Inoltra** e **Salva come nota** — stanno in una riga ancorata al bordo inferiore; finché un messaggio è aperto, la barra di navigazione si ritrae e le lascia lo spazio. Tieni premuta una riga per aprire il suo foglio: **Seleziona più elementi** viene per primo, poi le stesse azioni del menu contestuale del desktop — letto/non letto, contrassegna, sposta, rimanda, spam ed elimina.
...
**Una cartella esistente come vault (Android e iOS).** In **Vault → Crea un vault** c’è una terza via, **Una cartella su questo dispositivo**: scegli una cartella tenuta da un altro programma — Syncthing, l’app File, un secondo client di sincronizzazione — e Plainva legge e scrive lì senza copiare nulla. La cartella resta la cartella: se rimuovi il vault, sparisce solo il collegamento, i file restano. Le modifiche che un altro programma fa lì si vedono al ritorno nell’app e all’apertura di una nota. La sincronizzazione cloud è spenta per questi vault — una seconda sincronizzazione sullo stesso archivio sovrascriverebbe la prima — e la scheda nel dettaglio lo dice. Se l’accesso scade (cartella spostata, permesso revocato), la scheda lo segnala e **Ricollega cartella** lo ripristina.
...
**Differenze** apre la stessa vista di confronto del desktop: la nota a sinistra, la tua copia a destra, le righe uguali ripiegate e le stesse uscite — **adotta**, **tieni entrambe** (la copia si chiama allora `Nota (Version …).md`), **scarta copia**, ognuna chiede prima.
...
**Scorri una riga verso sinistra** per rivelare le sue azioni: **Segnalibro** ed **Elimina** su una nota, **Rinomina** ed **Elimina** su una cartella, **Elimina** su un database e nella casella. Sono le stesse azioni che la riga offre nel suo menu (tenere premuto) — lo scorrimento è solo la via più breve per arrivarci, mai l'unica. La prima volta te lo dice una striscia sopra l'elenco; la tocchi per farla sparire, e compare esattamente una volta per vault. **Nel calendario**, uno scorrimento a sinistra o a destra passa al periodo successivo o precedente (giorno, tre giorni, settimana, mese); un trascinamento che inizia in verticale scorre come sempre, e la riga di chip in alto scorre per conto suo.
...
## Notifiche sulle annotazioni

Quando qualcuno ha scritto su una nota, Plainva può avvisarti: gli stessi tre livelli e lo stesso interruttore di anteprima del desktop, in **Impostazioni → Contenuto e struttura**. Toccare il messaggio apre la nota ed evidenzia la scheda a cui si riferisce. Silenzi una singola nota con la campanella nel foglio dei commenti.
