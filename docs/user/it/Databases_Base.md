...
- **Giorno dal nome del file** (`file.day`) — calendari e linee del tempo possono collocare le note giornaliere in base al loro **nome di file**, senza che la data stia anche nel frontmatter. Il campo compare nel selettore del campo data non appena il vault dà un nome alle note giornaliere (cartella e formato sotto **Impostazioni → Il tuo vault → Contenuto e struttura**); viene calcolato dal nome, non sta in nessuna nota e non si può modificare, quindi una scheda collocata così non si può nemmeno trascinare su un altro giorno (rinominerebbe il file). Obsidian non conosce questo campo: una vista che lo usa lì resta vuota.
...
- **Colore della scheda**: una scheda della bacheca viene colorata con il colore proprio della nota — **Colore** nel menu della scheda lo imposta; è lo stesso colore mostrato dall'intestazione della nota e dalla sua scheda nella bacheca appunti. **Colore della scheda per** nelle impostazioni della **Vista** colora le altre schede con il colore dell'opzione di una proprietà Selezione/Stato/Selezione multipla; il colore proprio di una nota ha la precedenza.
- **Elenco cose da fare sulla scheda**: una scheda la cui nota contiene righe `- [ ]` mostra una barra di avanzamento con il conteggio; un clic apre le righe, dove puoi spuntarle o aggiungere un'attività secondaria nel campo sottostante (viene inserita dopo l'ultima casella della nota). Il conteggio è anche la colonna **Elenco cose da fare**, disponibile in ogni vista sotto **Proprietà**.
- **Corsie**: sotto **Vista** (sul telefono: **Configura**), **Corsie per** aggiunge un secondo asse — una riga per ogni valore di un'altra proprietà di selezione, stato o relazione (priorità, assegnatario…), con le stesse colonne in ogni riga. Trascinare una scheda su una cella scrive sia la proprietà della colonna sia quella della corsia; la freccia comprime una corsia, e Plainva lo ricorda per file. Sul telefono le corsie si impilano, e il chip di corsia su una scheda la sposta in un'altra corsia.
- **Limite WIP**: fai clic sul conteggio nell'intestazione di colonna e inserisci il numero massimo di schede che la colonna può contenere — l'intestazione mostrerà quindi `6/5` nel colore di avviso non appena la colonna lo supera. Le impostazioni della **Vista** (sul telefono: **Configura**) elencano i limiti di tutte le colonne; vuoto significa nessun limite.
