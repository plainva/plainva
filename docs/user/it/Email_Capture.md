...
- **Server e-mail (IMAP)** — per qualsiasi altro provider: host, porta e una **Password per app**. Per Gmail è `imap.gmail.com`, porta `993`, con una password per app da [myaccount.google.com/apppasswords](https://myaccount.google.com/apppasswords) (richiede l'autenticazione a due fattori) — nessun OAuth, nessuna verifica; l'assistente lo segnala da solo per gli indirizzi Gmail. Sono disponibili preimpostazioni già pronte per **web.de** e **GMX**. Aggiungi un host SMTP per inviare direttamente.
