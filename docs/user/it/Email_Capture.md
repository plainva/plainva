...
- **Server e-mail (IMAP)** — per qualsiasi altro provider: host, porta e una password oppure una **Password per app**. Sono disponibili preimpostazioni già pronte per provider di tutto il mondo — da **web.de**/**GMX** e **T-Online**, passando per **Orange**, **Libero**, **WP**, **Seznam** e **Comcast**, fino a **QQ Mail**, **NetEase**, **Naver** e **Yahoo! JAPAN**; il menu **Provider** ha per questo una riga di ricerca, e digitando il tuo indirizzo viene scelta automaticamente la preimpostazione corrispondente. Dove un provider ha delle particolarità, l'assistente lo segnala subito sotto il modulo: alcuni richiedono una **Password per app** o un **codice di autorizzazione** al posto della password dell'account, altri richiedono di attivare prima IMAP nelle impostazioni del provider — ciascuno con un link alla guida ufficiale. Per Gmail è `imap.gmail.com`, porta `993`, con una password per app da [myaccount.google.com/apppasswords](https://myaccount.google.com/apppasswords) (richiede l'autenticazione a due fattori) — un accesso separato con password per app; l'assistente lo segnala da solo per gli indirizzi Gmail. Le **caselle Outlook.com** non si possono più collegare tramite IMAP con password (Microsoft ha disattivato questa via) — la preimpostazione rimanda alla scheda **Microsoft**. **Proton Mail** funziona solo tramite il Proton Mail Bridge locale a pagamento (ha una propria preimpostazione). Aggiungi un host SMTP per inviare direttamente.
...
**Account e-mail senza password.** Una configurazione fallita durante il salvataggio poteva lasciare una voce senza password (Plainva 0.8.3 su desktop). L’area **E-mail** segnala una volta gli account che su questo dispositivo non hanno password e qui non hanno mai scaricato e-mail. **Vedi** mostra l’elenco. Se usi un account del genere su un altro dispositivo, scegli **Accedi su questo dispositivo**: in quel caso è la strada giusta. **Rimuovi** elimina la voce dopo una conferma, su tutti i dispositivi che sincronizzano le impostazioni di questo vault, mai e-mail o file. **Più tardi** nasconde l’avviso. Plainva non rimuove mai nulla da solo.
...
- **Con la tastiera nell'elenco** — Freccia su/giù seleziona il messaggio precedente/successivo e lo mostra nel lettore — se la freccia arriva su una conversazione compressa, la espande e seleziona il suo primo messaggio (verso il basso) o l'ultimo (verso l'alto); in una conversazione espansa le frecce percorrono i messaggi uno per uno —, Maiusc+freccia estende la selezione, Home/Fine saltano al primo/ultimo messaggio, freccia sinistra/destra comprime/espande una conversazione, Invio apre il messaggio nel lettore, Canc lo sposta nel cestino (con la stessa conferma del pulsante). Tutte le scorciatoie sono elencate nella finestra delle scorciatoie (F1) sotto **E-mail**.
...
Seleziona più messaggi per segnarli come letti o non letti, spostarli o eliminarli insieme. Le modifiche confermate aggiornano l’elenco. I messaggi non confermati restano selezionati, con un resoconto individuale. L’annullamento si applica dopo il lotto attuale. Ricarica la cartella prima di riprovare un’azione dall’esito incerto. Se il server non supporta un’operazione mirata, Plainva lo spiega e conserva gli altri messaggi.

**Con allegati** filtra i messaggi caricati. L’avviso indica quanti hanno metadati noti. Metadati mancanti non significano assenza di allegati; altre pagine possono contenere ulteriori risultati.

Con una tastiera fisica mobile, Su/Giù e Home/Fine spostano il focus nell’elenco. Invio apre o seleziona il messaggio. Sinistra/Destra comprimono o espandono le conversazioni; Esc esce dalla selezione o annulla i lotti successivi. I campi di ricerca mantengono i tasti di modifica.
