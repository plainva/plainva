...
**通常你不需要这个页面：** Plainva为OneDrive**和**Dropbox都自带了应用ID——你只需选择服务商，点击**连接**并登录即可。你只在两种情况下才需要**自己的**（免费）App注册：公司限制挡住了自带的访问方式，或者你想主动把连接掌握在自己手中。在**桌面端**，你可以在同步设置中通过**使用你自己的应用 ID**展开这些字段；在**手机**上，该字段就在连接表单中，紧跟在服务商下方。无论哪种方式，你都只需填入唯一一个公开值：
...
> 两种服务商自带的ID都已经预先填好——只有当你想使用**自己的**注册时，才需要第A部分或第B部分。
