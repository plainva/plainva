...
**通常你不需要这个页面：** Plainva自带了OneDrive和Dropbox的应用ID——你只需选择服务商，点击**连接**并登录即可。本指南适用于你想使用自己（免费）的App注册的情形：例如遇到公司限制，或者Plainva自带的Dropbox应用已达到用户数量上限，不再接受新的登录。在**桌面端**，你可以在同步设置中通过**使用你自己的应用 ID**展开这些字段；在**手机**上，该字段就在连接表单中，紧跟在服务商下方。无论哪种方式，你都只需填入唯一一个公开值：
