...
- **批量创建笔记**——生成带有OKF Frontmatter块（`type`，最好还带上`generated: { by: <your-tool>/<version>, at: … }`，以及你自己的属性）和Markdown正文的`.md`文件。自OKF 0.2起，`okf_version`只属于根目录的`index.md`，不再写入单篇笔记。这些文件一出现，Plainva就会为它们编制索引。
