...
如果 WebDAV、CalDAV 或 S3 服务返回登录页面或不完整的资源列表，Plainva 会报告同步错误。它不会将该响应视为空文件夹或空日历列表，也不会据此执行推断出的删除操作。此规则适用于桌面端和移动端。

如果本地文件因权限或读取错误而无法上传，任务及其错误会保留在队列中。请恢复文件访问权限后重新同步。只有确实不存在的文件才会被视为缺失；访问失败不会被记录为上传成功。

即使多次重命名，离线编辑也会保留。Plainva 会先完成相关移动，再上传当前内容；操作失败时，依赖它的任务会等待。如果远程源不存在，Plainva 会将本地仍保留的文件或文件夹内容上传到新位置。无法读取的内容会留在队列中并显示错误，以便重试。

删除确认仅适用于实际执行的那次删除。连接错误后重试或重启时，确认仍然有效；在同一位置新建的文件不会继承它。暂停批量删除时的额外确认和恢复选项，只影响当时显示的任务。更新前已在队列中的大量删除任务，可能需要再次确认。

两台设备在完成同步之前，把日志条目追加到同一篇每日笔记中，这并不是冲突：Plainva会按时间合并这些条目，并保留两台设备的每一行——即使两台设备各自独立创建了当天的笔记，同样如此。对笔记的其他任何同时更改，仍按以往的方式处理。参见[日志](Journal.md)。
