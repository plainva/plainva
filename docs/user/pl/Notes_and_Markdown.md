...
- **Tagi**: `#projekt` — albo zagnieżdżony `#projekt/strona` — w dowolnym miejscu tekstu jest tagiem. Jest rysowany jako mała pigułka podczas pisania i czytania, a kliknięcie go otwiera notatki, które go noszą (na telefonie: dotknięcie w trybie czytania). Co liczy się jako tag i jak tagi dostają kolor: [Wyszukiwanie](Search.md).
...
- W trybie ikon wybierasz u góry **kolor** — ta sama paleta co przy pasku nagłówka, **A** dla koloru domyślnego i krążek z pierścieniem barw (**Własny kolor …**) dla dowolnej wartości; obowiązuje dla ikony, którą klikniesz zaraz potem. Ta sama siatka pojawia się wszędzie, gdzie Plainva pyta o kolor — także na telefonie.
...
- **Dźwięk się odtwarza**: osadzony plik audio (`.m4a`, `.mp3`, `.wav`, `.ogg`, `.opus`, `.webm`, `.flac`, `.aac`) pokazuje odtwarzacz zamiast swojej nazwy — w edytorze, w trybie czytania, na kartach tablicy i w wierszach dziennika. Dlatego dołączony dźwięk jest osadzany jak obraz (`![[…]]`), a nie tylko linkowany. Plainva nie odtwarza wideo; `.mp4` pozostaje linkiem.
...
- **Pliki tekstowe** otwierają się w samej Plainvie: `.txt`, `.csv`, `.json`, `.yaml`, kod źródłowy i podobne. Jeśli Twój sejf potrzebuje więcej, dopisz rozszerzenia w **Ustawienia → Twój sejf → Treść i struktura → Więcej plików tekstowych** — lista może tylko dodawać, nigdy odbierać. Jeśli początek pliku pokaże, że to jednak nie tekst, Plainva go nie wyświetli i zaproponuje program domyślny: wyświetlenie i zapisanie uszkodziłoby plik. Tak otwarty plik zachowuje przy zapisie swoje końce wierszy i BOM — należy do Ciebie, nie do Plainvy. Podczas edycji Plainva koloruje tekst na podstawie rozszerzenia pliku — to samo podświetlanie, jakie otrzymuje blok kodu wewnątrz notatki. Narzędzia notatki pozostają wyłączone: brak Podglądu na żywo, brak nagłówka właściwości, brak `[[links]]`, brak menu "/". Znajdź i zamień pozostaje, ponieważ to funkcja tekstu, a nie funkcja notatki.
