...
**Konta na wszystkich Twoich urządzeniach** to trzy kroki, które się uzupełniają. **1 · Szyfrowanie**: chroni to, co wędruje między urządzeniami, ale samo niczego nie przenosi. **2 · Ustawienia i konta**: zapisuje ustawienia sejfu *oraz Twoje konta* (kalendarze, skrzynki pocztowe, wybór kalendarzy) w małym pliku w sejfie. **3 · Przenoszenie logowań**: dodatkowo przenosi statyczne hasła IMAP i CalDAV, zaszyfrowane. Krok 3 można włączyć dopiero wtedy, gdy działa krok 2 — hasło może trafić tylko do konta, które urządzenie już zna. Nie są przenoszone: ścieżki specyficzne dla urządzenia oraz logowania OAuth (Microsoft, Google); ich tokeny są powiązane z urządzeniem, więc konto pojawia się na nowym urządzeniu i wymaga tam jednorazowego **Zaloguj się**.
