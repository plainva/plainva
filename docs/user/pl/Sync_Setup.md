...
**Konta na wszystkich Twoich urządzeniach** to trzy kroki. **1 · Ustawienia i konta**: zapisuje ustawienia sejfu *oraz Twoje konta* (kalendarze, skrzynki pocztowe, wybór kalendarzy) w małym pliku w sejfie — do tego nie jest potrzebne **żadne** hasło. **2 · Hasło synchronizacji** (opcjonalnie): potrzebne tylko wtedy, gdy mają wędrować także logowania; dodatkowo szyfruje ustawienia z kroku 1. **3 · Przenoszenie logowań**: dodatkowo przenosi statyczne hasła IMAP i CalDAV, zaszyfrowane, i można je włączyć dopiero, gdy działa krok 1 i hasło jest odblokowane — hasło może trafić tylko do konta, które urządzenie już zna. Nie są przenoszone: ścieżki specyficzne dla urządzenia oraz logowania OAuth (Microsoft, Google); ich tokeny są powiązane z urządzeniem, więc konto pojawia się na nowym urządzeniu i wymaga tam jednorazowego **Zaloguj się**.
