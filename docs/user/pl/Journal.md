...
- **Nastrój:** jeśli ustawienie **Nastrój: właściwość notatki dziennej** (to samo miejsce) nosi nazwę — na przykład `nastroj` — nagłówek dnia w dzienniku pokazuje pięć kropek i oceniasz dzień jednym naciśnięciem. W notatce dziennej stoi wtedy `nastroj: 4`; naciśnięcie bieżącej wartości ją kasuje. Tę samą właściwość można prowadzić w bazie jako kolumnę typu **Ocena** i tam sortować. Zostaw puste, a ten sejf nie ocenia dni.
...
- **Miejsce:** domyślnie wyłączone. Włącz **Pozwól dodawać miejsce do wpisów dziennika** (Ustawienia → Wygląd), a pole zapisu zyska drugi przycisk; jedno naciśnięcie dodaje Twoje współrzędne do wpisu we własnym wierszu: `📍 52.5200, 13.4050`. Cztery miejsca po przecinku to około jedenastu metrów — dość na budynek, za mało na pokój. Nazwę miejsca możesz dopisać sam albo usunąć wiersz; Plainva nie pyta o nią żadnej usługi online. **Nic nie jest zapisywane po cichu**: żadnej historii, żadnej mapy, żadnego śledzenia w tle — tylko ta jedna pozycja w chwili naciśnięcia. System pyta o zgodę przy pierwszym naciśnięciu. Na telefonie przycisk jest zawsze; na komputerze tylko tam, gdzie system operacyjny daje aplikacji pozycję.
