...
- **Dzień z nazwy pliku** (`file.day`) — kalendarze i osie czasu mogą umieszczać notatki dzienne według ich **nazwy pliku**, bez zapisywania daty także we frontmatterze. Pole pojawia się w wyborze pola daty, gdy tylko sejf nazywa notatki dzienne (folder i format w **Ustawienia → Twój sejf → Treść i struktura**); jest wyliczane z nazwy, nie stoi w żadnej notatce i nie da się go edytować — karty umieszczonej w ten sposób nie przeciągniesz też na inny dzień (to zmieniłoby nazwę pliku). Obsidian nie zna tego pola: widok, który go używa, pozostaje tam pusty.
...
- **Kolor karty**: karta na tablicy jest zabarwiana własnym kolorem notatki — ustawia go **Kolor** w menu karty; to ten sam kolor, który pokazują nagłówek notatki i jej karta na tablicy korkowej. **Kolor karty według** w ustawieniach **Widoku** barwi pozostałe karty kolorem opcji właściwości typu Wybór/Status/Wielokrotny wybór; własny kolor notatki ma pierwszeństwo.
- **Lista zadań na karcie**: karta, której notatka zawiera linie `- [ ]`, pokazuje pasek postępu z licznikiem; kliknięcie rozwija linie, gdzie możesz je odhaczyć albo dodać podzadanie w polu poniżej (trafia ono za ostatnim polem wyboru notatki). Licznik jest jednocześnie kolumną **Lista zadań**, dostępną w każdym widoku pod **Właściwościami**.
- **Tory**: w **Widoku** (na telefonie: **Konfiguruj**) opcja **Tory według** dodaje drugą oś — jeden wiersz na każdą wartość innej właściwości typu wybór, status lub relacja (priorytet, osoba przypisana…), z tymi samymi kolumnami w każdym wierszu. Przeciągnięcie karty na komórkę zapisuje zarówno właściwość kolumny, jak i toru; strzałka zwija tor, a Plainva zapamiętuje to dla każdego pliku. Na telefonie tory układają się jeden pod drugim, a etykieta toru na karcie przenosi ją do innego toru.
- **Limit WIP**: kliknij licznik w nagłówku kolumny i wpisz maksymalną liczbę kart, jaką kolumna może pomieścić — nagłówek pokaże wtedy `6/5` w kolorze ostrzegawczym, gdy tylko kolumna go przekroczy. Ustawienia **Widoku** (na telefonie: **Konfiguruj**) wymieniają limity wszystkich kolumn; puste pole oznacza brak limitu.
...
- **Komentarze przy komórce**: jeśli właściwość nosi uwagi, jej komórka pokazuje małą kropkę z liczbą otwartych wątków — w widoku **Tabela**, **Tablica** i **Galeria**. Komentarz wisi przy **notatce i jej kluczu właściwości**, nie przy `.base`: ta sama uwaga pojawia się dlatego w każdej bazie danych, która pokazuje tę notatkę, oraz w panelu **Właściwości** samej notatki. **Skomentuj tę właściwość** w menu komórki rozpoczyna nowy wątek. Przy zmianie nazwy kolumny komentarze przenoszą się wraz z nią; po usunięciu właściwości karta pozostaje czytelna i podaje zapisaną wówczas wartość. Więcej w [Bezpieczeństwo i udostępnianie](Security_and_Sharing.md).
