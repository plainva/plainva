...
- **Listy zadań**: jeśli baza danych jest bazą zadań i masz połączone konto kalendarza/zadań, **Konfiguruj → Źródło danych** pokazuje wiersz **Twórz też nowe zadania w**. Wybierz tam listę, a zadanie utworzone w Plainva zostanie utworzone też na tej liście u dostawcy; bez wyboru pozostaje zwykłą notatką, dokładnie jak dotychczas. Wybór należy do bazy danych (zapisany jako `plainva.taskList`, patrz [Dokumentacja formatu plików](File_Format_Reference.md)), a nie do pojedynczego zadania, i wiersz pojawia się tylko wtedy, gdy konto rzeczywiście oferuje listę zadań. Jeśli wybrana lista później zniknie (konto usunięte, lista usunięta), Plainva nie tworzy zadania gdzie indziej — traktuje bazę danych tak, jakby nic nie zostało wybrane.
