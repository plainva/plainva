...
- **Koloruj tagi**: w **Ustawienia → Aplikacja → Wygląd** włącz **Koloruj tagi**, a każdy tag dostanie kolor wynikający z jego nazwy — tagi zagnieżdżone dzielą kolor tagu najwyższego poziomu — w notatkach, właściwościach, bazach danych i na liście tagów. Domyślnie wyłączone; dotyczy tego urządzenia i niczego nie zapisuje.
