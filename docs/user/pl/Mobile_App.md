...
- **Wszystkie obszary** są zawsze dostępne przez **arkusz obszarów**: przez **Obszary** na pasku albo **przytrzymanie paska**. Pokazuje TWÓJ układ — najpierw obszary paska, potem reszta, w kolejności, którą ustawiłeś —, zaznacza bieżący i na dole prowadzi wprost do **Dostosuj pasek nawigacji…**. Tagi, zakładki i ostatnio otwarte nie są już osobnymi obszarami — leżą pod **Notatkami**.
- **Ustawianie paska nawigacji:** **Ustawienia** → **Paski i obszary**. Przyciskami **−**/**+** decydujesz, ile powierzchni pokazuje pasek (2–4, z podglądem na żywo). Każdy wiersz odpowiada: dotknięcie wsuwa obszar na pasek albo go zdejmuje, **oko** obok mówi, jak jest, a **uchwyt** przestawia przeciąganiem — przy górnej i dolnej krawędzi lista przewija się razem, więc jeden ruch wystarcza. Ukrycie niczego nie traci: to, czego nie ma na pasku, wciąż otworzysz przez **Obszary**. Jeśli otwarty obszar zejdzie z paska, aplikacja przechodzi do pierwszego widocznego. Ten sam pasek ułożysz też **na komputerze** (Ustawienia → Skarbiec → Paski i obszary); przy włączonej synchronizacji ustawień układ wędruje między urządzeniami.
...
- **＋** unosi się jako okrągły przycisk nad paskiem i otwiera szybkie tworzenie w dwóch grupach: notatka, „Z szablonu…”, notatka dzienna, wpis dziennika, folder, baza danych — a poniżej wydarzenie i zadanie, tworzone w kalendarzu i na liście zadań. Menu **Nowy** na komputerze oferuje te same pozycje w tej samej kolejności.
...
Tuż pod notatką dzienną znajduje się **Dziennik**: wpisy wybranego dnia, pole, które zapisuje dokładnie w tym dniu, oraz **Wszystkie dni**, które otwiera ekran dziennika ze strumieniem wszystkich dni. **＋** oferuje **Wpis dziennika** z dowolnego miejsca — zobacz [Dziennik](Journal.md).
...
Kalendarze, które Twój telefon już zna, łączysz bez logowania: **Ustawienia → Konta w chmurze → Połącz konto…** oferuje **Na tym urządzeniu** jako pierwszy kafelek, dopóki nie ma jeszcze konta urządzenia. Dotknięcie prosi o uprawnienie systemowe; potem wszystkie kalendarze urządzenia są na liście, a na iOS listy przypomnień jako listy zadań. Karta konta pokazuje stan uprawnienia i prowadzi do ustawień systemu, gdy go brakuje; **Usuń konto** tylko rozłącza, kalendarze urządzenia pozostają.
...
**Przesuń wiersz w lewo**, aby odsłonić jego akcje: **Zakładka** i **Usuń** przy notatce, **Zmień nazwę** i **Usuń** przy folderze, **Usuń** przy bazie danych i w skrzynce. To te same akcje, które wiersz oferuje w swoim menu (długie przytrzymanie) — przesunięcie to tylko krótsza droga do nich, nigdy jedyna. Za pierwszym razem informuje o tym pasek nad listą; odsuwasz go dotknięciem, a pojawia się dokładnie raz na vault. **W kalendarzu** przesunięcie w lewo lub w prawo przechodzi do następnego lub poprzedniego okresu (dzień, trzy dni, tydzień, miesiąc); przeciągnięcie zaczynające się pionowo przewija jak zwykle, a rząd chipów powyżej przewija się osobno.
