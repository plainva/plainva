...
削除の確認は、実際に実行された削除だけに適用されます。接続エラー後の再試行や再起動後も保持されますが、同じ場所に新しく作成されたファイルには引き継がれません。一時停止された大量削除の追加確認と復元の選択は、その画面に表示された処理だけが対象です。更新前からキューに残っていた大量削除では、もう一度確認が必要になる場合があります。

二つの端末が、同期する前に同じデイリーノートへ追加したジャーナルエントリは、競合になりません: Plainvaは時刻順にそれらを統合し、両方の端末のすべての行を保持します——両方の端末がその日のノートを別々に作成していた場合も同様です。ノートに対するそれ以外の同時変更は、これまでどおり扱われます。[ジャーナル](Journal.md)を参照してください。
