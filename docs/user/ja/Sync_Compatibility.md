...
WebDAV、CalDAV、S3サービスからログインページや不完全な一覧が返された場合、Plainvaは同期エラーを表示します。その応答を空のフォルダーや空のカレンダー一覧として扱い、削除を推定することはありません。デスクトップとモバイルの両方に適用されます。

権限または読み取りのエラーでローカルファイルをアップロードできない場合、処理とエラーはキューに残ります。ファイルへのアクセスを復旧してから、同期を再試行してください。実際に存在しないファイルだけを欠落として扱い、アクセスエラーをアップロード成功として記録することはありません。

オフラインでの編集は、名前を何度変更しても保持されます。Plainva は関連する移動を完了してから最新の内容をアップロードします。処理が失敗すると、それに依存する処理は待機します。リモートの移動元が存在しない場合は、ローカルに残っているファイルまたはフォルダーの内容を新しい場所にアップロードします。読み取れない内容は、再試行できるようエラー付きでキューに残ります。
...
| OneDrive | 利用可能 | ブラウザー経由のログイン（PKCE、シークレット不要）。Plainvaは独自のアプリ登録をあらかじめ用意しています——OneDriveを選んで接続するだけで、設定は不要です。自前の（無料の）Entraアプリ登録を使うことも引き続き任意で選べます（[OneDrive & Dropbox (BYO)](OneDrive_and_Dropbox_BYO_Guide.md)ガイドを参照）。 |
| Dropbox | 利用可能 | ブラウザー経由のログイン（PKCE、シークレット不要）。Plainvaは独自のDropboxアプリをあらかじめ用意しています——Dropboxを選んで接続するだけで、設定は不要です。自前の（無料の）Dropboxアプリを使うことも引き続き任意で選べます（[OneDrive & Dropbox (BYO)](OneDrive_and_Dropbox_BYO_Guide.md)ガイドを参照）。 |
