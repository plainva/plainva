...
**OneDriveでは、通常このページは必要ありません:** PlainvaはOneDrive用の独自のアプリIDをあらかじめ用意しています——プロバイダーを選び、**接続**をクリックしてサインインするだけです。**Dropboxでは、現在このページが必要です:** Plainvaに同梱されているDropboxアプリがユーザー数の上限に達し、新規のサインインを受け付けなくなりました。この状況が解消されるまで、Dropboxは自分の（無料の）アプリ登録でのみ接続できます。社内の制限で同梱のアクセスが塞がれている場合は、OneDriveも同様です。**デスクトップ**では、同期の設定で**独自のアプリ ID を使用**から入力欄を表示します。**スマートフォン**では、その項目はプロバイダーのすぐ下、接続フォームの中に直接あります。いずれの場合も、公開値を1つだけ入力します。
...
> OneDriveについては、同梱のIDがすでに入力済みです——パートAは、**自分自身の**登録を行う場合にのみ必要です。Dropboxについては、現時点ではパートBのみが有効な手順です。
