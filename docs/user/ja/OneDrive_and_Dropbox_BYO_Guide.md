...
**通常、このページは必要ありません:** PlainvaはOneDriveとDropboxの独自のアプリIDをあらかじめ用意しています——プロバイダーを選び、**接続**をクリックしてサインインするだけです。このガイドは、自分の（無料の）アプリ登録を使いたい場合のためのものです——社内の制限がある場合や、Plainvaに同梱されているDropboxアプリがユーザー数の上限に達し、新規のサインインを受け付けなくなった場合などです。**デスクトップ**では、同期の設定で**独自のアプリ ID を使用**から入力欄を表示します。**スマートフォン**では、その項目はプロバイダーのすぐ下、接続フォームの中に直接あります。いずれの場合も、公開値を1つだけ入力します。
